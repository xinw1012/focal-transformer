...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, window_size_glo, focal_window, 
        ...
        self.register_buffer("mask_rolled", mask_rolled)
    ...
    def forward(self, x_all, mask_all=None):
        ...
        # print("time cost for qkv(x): ", time.time() - tic)
        # tic = time.time()
        # partition q map
        ...
        if self.expand_size > 0 and x_all[1] is not None:
            ...
            qkv_expanded = self.kv(x_expanded).reshape(x_expanded.shape[0], -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4).contiguous()
            k_expanded, v_expanded = qkv_expanded[0], qkv_expanded[1]
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(self.focal_level-1):
                x_window_pooled = x_all[k+2]  # B, nWh, nWw, C
                ...
                (k_pooled_k, v_pooled_k) = map(
                    lambda t: t.reshape(-1, self.focal_window**2, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous(), 
            ...
            k_pooled = torch.cat(k_pooled, 2)
            v_pooled = torch.cat(v_pooled, 2)
        ...
        if self.focal_level > 1 and self.pool_method != "none": 
            ...
            for k in range(self.focal_level-1):     
                ...
                if self.pool_method == "mean":
                    ...
                    x_windows_noreshape = x_windows_noreshape.view(B, nWh, nWw, window_size_glo*window_size_glo, C).permute(0, 1, 2, 4, 3).contiguous() # B, nWh, nWw, C, wsize**2
...
if __name__ == '__main__':
    ...
    model = FocalTransformer(embed_dim=96, depths=[2,2,6,2], drop_path_rate=0.2, 
        focal_levels=[2,2,2,2], expand_sizes=[3,3,3,0], num_heads=[3,6,12,24]).cuda()
    flops = model.flops()
