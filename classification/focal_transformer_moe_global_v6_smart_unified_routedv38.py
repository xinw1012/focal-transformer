        ...
        if self.pool_method != "none": 
            ...
            for l in range(self.focal_level):         
                if self.pool_method == "conv":
                    ...
                    k_k = k.permute(0, 2, 3, 1).contiguous().view(B, nH, nW, self.num_heads, 1, C // self.num_heads) # B, nWh, nWw, C       
                    
                    v = self.pool_layers[l](v)
                    v_k = v.permute(0, 2, 3, 1).contiguous().view(B, nH, nW, self.num_heads, 1, C // self.num_heads) # B, nWh, nWw, C       
                    
                elif self.pool_method == "conv_share":
                    k = self.pool_layers[l](k)                    
                    x_p_k = x_p.view(B, C, *x_p.shape[2:]).permute(0, 2, 3, 1).contiguous().view(B, nH, nW, self.num_heads, 1, C // self.num_heads) # B, nWh, nWw, C           
                k_all.append(k_k)
                v_all.append(v_k)

        import pdb; pdb.set_trace()
        
        k_all.append(k_all[-1].mean(1, keepdim=True).mean(2, keepdim=True).expand(-1, nH, nW, -1, -1, -1))
        v_all.append(v_all[-1].mean(1, keepdim=True).mean(2, keepdim=True).expand(-1, nH, nW, -1, -1, -1))
