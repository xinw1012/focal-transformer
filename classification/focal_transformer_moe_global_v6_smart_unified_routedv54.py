...
def window_reverse(windows, window_size, H, W):
    ...
    Returns:
        x: (B, C, H, W)
    """
    B, C = windows.shape[0:2]
    x = windows.permute(0, 1, 2, 4, 3, 5).contiguous().view(B, C, H, W)
    return x
...
class FocalConvolution(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, focal_factor, 
        ...
        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)
        self.pool_filters = nn.ParameterList()
        ...
        self.focal_weights = nn.Parameter(torch.zeros(self.focal_level+1))
        self.kernel_sizes = []
        self.stride_sizes = []
        self.padding_sizes = []
        if self.pool_method != "none":
            for k in range(self.focal_level):
                ...
                kernel_size = self.focal_window if k == 0 else 3 # max(self.focal_window-self.focal_factor*k, 3)     
                stride = 1 # if k == 0 else self.focal_stride 
                self.stride_sizes.append(stride)
                if self.pool_method == "conv":
                    ...
                    if kernel_size < min(input_resolution):       
                        self.pool_filters.append(
                            nn.Parameter(torch.zeros(dim, 1, kernel_size, kernel_size))
                        )      
                        trunc_normal_(self.pool_filters[-1], std=.02)
                        self.kernel_sizes.append(kernel_size)      
                        self.padding_sizes.append(kernel_size//2 - stride // 2)  
                    else:
                        self.pool_filters.append(
                            nn.Parameter(torch.zeros(dim, 1, input_resolution[0], input_resolution[1]))
                        )  
                        trunc_normal_(self.pool_filters[-1], std=.02)
                        self.kernel_sizes.append(min(input_resolution))      
                        self.padding_sizes.append(0)  
                    ...
                    if kernel_size < min(input_resolution):       
                        ...
                        self.pool_filters.append(
                            nn.Parameter(torch.zeros(1, 1, input_resolution[0], input_resolution[1]))
                        )  
                        trunc_normal_(self.pool_filters[-1], std=.02)
                        self.kernel_sizes.append(min(input_resolution))      
                        self.padding_sizes.append(0)  
        ...
        #     # we need positional embedding to indicate the relative position between target location and the srided window
        #     self.pos_embed = nn.Parameter(torch.zeros(1, dim, 1, 1, self.focal_stride, self.focal_stride))
    ...
    def forward(self, x, mask_all=None):
        ...
        modulator = 0
        focal_weights_normalized = self.focal_weights
        if self.pool_method != "none": 
            # if we add coarser granularity and the pool method is not none
            for l in range(self.focal_level):         
                if self.pool_method in ["conv", "conv_share"]:                    
                    pool_filter = self.pool_filters[l] if self.pool_method == "conv" else self.pool_filters[l].expand(C, -1, -1, -1)
                    x4conv_kv = self.act(F.conv2d(x4conv_kv, pool_filter, groups=C, 
                            stride=self.stride_sizes[l], padding=self.padding_sizes[l]))
                    modulator = modulator + x4conv_kv*focal_weights_normalized[l]
        
        x_global = self.act(x4conv_kv.mean(2, keepdim=True).mean(3, keepdim=True))
        modulator = modulator + x_global*focal_weights_normalized[-1]
