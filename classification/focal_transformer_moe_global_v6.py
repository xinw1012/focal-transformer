    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):        
            ...
            if self.conv_route:
                ...
                x_k_route_scores_k = self.routers[l_k](x_k4conv).view(B, 1, -1).transpose(1, 2)
            else:
                x_k_route_scores_k = self.routers[l_k](x_k)
                x_k_route_scores_k = x_k_route_scores_k.view(B, -1, 1) # B, nH, nW, 1

            # x_k_route_scores = torch.softmax(x_k_route_scores, dim=1)

            if l_k == 0:
                x_k_route_scores_all = x_k_route_scores_k.view(B, x_k.shape[1], x_k.shape[2], 1)
            ...
            k_k_selected = torch.gather(k_k, 1, topk_index_k.view(B, -1, 1).unsqueeze(-1).repeat(1, 1, self.num_heads, C // self.num_heads))
            v_k_selected = torch.gather(v_k, 1, topk_index_k.view(B, -1, 1).unsqueeze(-1).repeat(1, 1, self.num_heads, C // self.num_heads))

            # k_k_selected_a = k_k_selected.view((-1,) + topk_index_k.shape[2:] + (C // self.num_heads,)).transpose(1, 2)
            # v_k_selected_a = v_k_selected.view((-1,) + topk_index_k.shape[2:] + (C // self.num_heads,)).transpose(1, 2)
        ...
        if self.focal_level > 0:
            ...
            for k in range(self.focal_level):
                ...
                relative_position_bias_to_windows = relative_position_bias_to_windows.view(-1, 1, 1, topKs[k])
