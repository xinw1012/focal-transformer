    ...
    def forward(self, x_all, mask_all=None):
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(self.focal_level-1):
                ...
                qkv_pooled = self.qkv(x_window_pooled).reshape(B, nWh, nWw, 3, C).permute(3, 0, 1, 2, 4).contiguous()
                k_pooled_k, v_pooled_k = qkv_pooled[1], qkv_pooled[2]  # B, nWh, nWw, C
                (k_pooled_k, v_pooled_k) = map(
                    lambda t: t.permute(0, 3, 1, 2).contiguous(), (k_pooled_k, v_pooled_k)
                )

                (k_pooled_k, v_pooled_k) = map(
                    lambda t: self.unfolds[k](t).view(
                    B, C, self.focal_window, self.focal_window, -1).permute(0, 4, 2, 3, 1).contiguous().\
                    view(-1, self.focal_window**2, C), 
                    (k_pooled_k, v_pooled_k)  # (B x (nH*nW)) x unfold_wsize x unfold_wsize x C
                )

                (k_pooled_k, v_pooled_k) = map(
                    lambda t: t.reshape(-1, self.focal_window**2, self.num_heads, C // self.num_heads).transpose(1, 2), 
...
class HeadAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, window_size_glo, focal_window, 
        ...
        self.qkv_xhead = nn.Linear(dim, dim * 3, bias=qkv_bias)
        ...
        self.relative_position_bias_xhead =  nn.Parameter(
                torch.zeros(
                    self.num_heads,
                    self.num_heads,
                    )
    ...
    def forward(self, x):
        ...
        # x = x.reshape(B, nW, self.num_heads, C // self.num_heads)

        qkv_xhead = self.qkv_xhead(x).view(B, nW, 3, self.num_heads, C // self.num_heads)

        q_windows_xheads, k_windows_xheads, v_windows_xheads = \
            qkv_xhead[:, :, 0], qkv_xhead[:, :, 1], qkv_xhead[:, :, 2]
        
        attn_xhead = (q_windows_xheads @ k_windows_xheads.transpose(-2, -1)) # B*nW, window_size*window_size, nHead, nHead
        attn_xhead = attn_xhead.mean(1, keepdim=True)
        attn_xhead = attn_xhead + self.relative_position_bias_xhead[None, None, :, :]         
        attn_xhead = self.softmax(attn_xhead)
        attn_xhead = self.attn_drop(attn_xhead)
        x = (attn_xhead @ v_windows_xheads)
        x = x.reshape(x.shape[0], nW, C)
        x = self.proj(x)
        x = self.proj_drop(x)
...
class FocalTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.focal_level > 1 and self.pool_method != "none": 
            ...
            for k in range(self.focal_level-1):     
                ...
                if H > H_pool:
                    ...
                    x_level_k = torch.pad(x_level_k, (0,0,0,0,pad_t,pad_b))
                ...
                if W > W_pool:
                    ...
                    x_level_k = torch.pad(x_level_k, (0,0,pad_l,pad_r))
        ...
        # Head Attention
        x = x + self.drop_path(self.headattn(self.norm2(x)))        
        # FFN
...
if __name__ == '__main__':
    ...
    model = FocalTransformer(embed_dim=96, depths=[2,2,6,2], drop_path_rate=0.2, 
