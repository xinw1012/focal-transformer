...
#         self.register_buffer("gaussian_noise", torch.Tensor(16, 24*24+1).fill_(0.0))

#     def forward(self, x):
#         B, N, C = x.shape
#         # determine the route based on x
#         route = self.router(x).flatten(-2)
#         noise = self.gaussian_noise.normal_(0.0, 1.0/196)
#         route = route + noise
#         score = torch.softmax(route, dim=-1)
#         topk_score, topk_index = torch.topk(score, self.topK, dim=-1)

#         x_selected = x.view(-1, C)[topk_index + N*torch.arange(B).view(B, 1).type_as(topk_index)]
#         x_selected = x_selected.contiguous().view(B, self.topK, C)
#         N_selected = x_selected.shape[1]

#         q = self.q(x).reshape(B, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)[0]
#         kv = self.kv(x_selected).reshape(B, N_selected, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
#         k, v = kv[0], kv[1]   # make torchscript happy (cannot use tensor as tuple)

#         attn = (q @ k.transpose(-2, -1)) * self.scale
#         attn = attn + topk_score[:, None, None, :]
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, window_size_glo, focal_window, 
        ...
        if pool_method != "none" and focal_level > 1:
            ...
            for k in range(focal_level-1):
                ...
                self.unfolds += [nn.Unfold(
                    ...
                    stride=stride, padding=self.focal_window // 2)
                ]
                relative_position_bias_table_to_windows = nn.Parameter(
                    torch.zeros(
                        self.num_heads,
                        (self.window_size[0] + self.focal_window - 1) * (self.window_size[1] + self.focal_window - 1), 
                        )
                ) # 2*Wh-1 * 2*Ww-1, nH
                ...
                relative_position_index_k = get_relative_position_index(self.window_size, to_2tuple(self.focal_window))
                self.register_buffer("relative_position_index_{}".format(k), relative_position_index_k)

        self.q = nn.Linear(dim, dim * 1, bias=qkv_bias)
        self.kv = nn.Linear(dim, dim * 2, bias=qkv_bias)

        self.q_router = nn.Linear(dim, 1)
        self.kv_router = nn.Linear(dim, self.num_heads)
        ...
        self.topK = max(32, min(self.input_resolution[0], self.input_resolution[1]))
        ...
        for k in range(self.focal_level):
            ...
            #     torch.zeros(
            #         self.num_heads,
            #         (self.nWh + self.input_resolution[0] - 1) * (self.nWw + self.input_resolution[1] - 1), 
            #         )
            # ) # 2*Wh-1 * 2*Ww-1, nH
            # trunc_normal_(relative_position_bias_table_to_windows, std=.02)
            # self.relative_position_bias_table_to_windows.append(relative_position_bias_table_to_windows)

            # relative_position_index_k = get_relative_position_index((self.nWh, self.nWw), self.input_resolution)
            # self.register_buffer("relative_position_index_{}".format(k), relative_position_index_k)

            router_k = nn.Linear(dim, self.num_heads)
    ...
    def forward(self, x_all, mask_all=None):
        ...
        x_windows = window_partition_noreshape(x, self.window_size[0])
        x_windows = x_windows.view(x_windows.shape[:3] + (-1, C))
        # get routing scores
        q_routing_score = self.q_router(x_windows)
        kv_routing_score = self.kv_router(x_windows)

        topk_score_q, topk_index_q = torch.topk(
            q_routing_score, (self.window_size[0]-2)*(self.window_size[1]-2), dim=-2) # B, topK, 1

        topk_score_kv, topk_index_kv = torch.topk(
            kv_routing_score, (self.window_size[0]-2)*(self.window_size[1]-2), dim=-2) # B, topK, nHeads            

        x_windows4q = torch.gather(x_windows, -2, topk_index_q.flatten(-2).unsqueeze(-1).repeat(1,1,1,1,C)
            ).view(x_windows.shape[:3] + (-1, C))

        x_windows4kv = torch.gather(x_windows, -2, topk_index_kv.flatten(-2).unsqueeze(-1).repeat(1,1,1,1,C)
            ).view(x_windows.shape[:3] + (-1, self.num_heads, C))

        q_windows = self.q(x_windows4q).view(x_windows4q.shape[:-1] + (self.num_heads, C // self.num_heads))
        kv_windows = self.kv(x_windows4kv).view(x_windows4kv.shape[:-1] + (2, self.num_heads, C // self.num_heads))
        k_windows, v_windows = kv_windows[:, :, :, :, :, 0], kv_windows[:, :, :, :, :, 1]

        q_windows = q_windows.permute(0, 1, 2, 4, 3, 5).contiguous().view(
            -1, self.num_heads, (self.window_size[0]-2)*(self.window_size[1]-2), C // self.num_heads)

        k_windows = torch.diagonal(k_windows, dim1=4, dim2=5).permute(0, 1, 2, 5, 3, 4).contiguous().view(
            -1, self.num_heads, (self.window_size[0]-2)*(self.window_size[1]-2), C // self.num_heads)

        v_windows = torch.diagonal(v_windows, dim1=4, dim2=5).permute(0, 1, 2, 5, 3, 4).contiguous().view(
            -1, self.num_heads, (self.window_size[0]-2)*(self.window_size[1]-2), C // self.num_heads)


        k_all = k_windows; v_all = v_windows; attention_bias = []; attention_index = []

        for l_k in range(self.focal_level):
            x_k = x_all[l_k]
            x_k_route_scores = self.routers[l_k](x_k).view(B, -1, self.num_heads)  # B, nH, nW, nHeads

            # if self.training:
            #     noise = x_k_route_scores.clone().normal_(0.0, std=1.0/self.dim)
            #     x_k_route_scores = x_k_route_scores + noise

            # x_k_route_probs = torch.softmax(x_k_route_scores, dim=1)
            topk_score_k, topk_index_k = torch.topk(x_k_route_scores, self.topK, dim=1) # B, topK, nHeads
            attention_bias.append(topk_score_k)
            attention_index.append(topk_index_k)

            x_k_selected = torch.gather(x_k.view(B, -1, x_k.shape[-1]), 1, topk_index_k.flatten(1).unsqueeze(-1).repeat(1,1,C))
            x_k_selected = x_k_selected.view(B, self.topK, self.num_heads, C)

            q_k_selected = self.q(x_k_selected).view(x_k_selected.shape[:-1] + (self.num_heads, C // self.num_heads))
            kv_k_selected = self.kv(x_k_selected).view(x_k_selected.shape[:-1] + (2, self.num_heads, C // self.num_heads))
            k_k_selected, v_k_selected = kv_k_selected[:, :, :, 0], kv_k_selected[:, :, :, 1]

            k_k_selected = torch.diagonal(k_k_selected, dim1=2, dim2=3).permute(0, 3, 1, 2).unsqueeze(1).repeat(1, nH * nW // (self.window_size[0]*self.window_size[1]), 1, 1, 1)
            v_k_selected = torch.diagonal(v_k_selected, dim1=2, dim2=3).permute(0, 3, 1, 2).unsqueeze(1).repeat(1, nH * nW // (self.window_size[0]*self.window_size[1]), 1, 1, 1)

            k_all = torch.cat((k_all, k_k_selected.view(-1, self.num_heads, self.topK, C // self.num_heads)), 2)
            v_all = torch.cat((v_all, v_k_selected.view(-1, self.num_heads, self.topK, C // self.num_heads)), 2)
        
        if self.pool_method != "none" and self.focal_level > 1:
            k_pooled = []
            v_pooled = []
            for k in range(self.focal_level-1):
                # NOTE: For k >= 1, the unfolding window is not exactly centered at each window. To achieve this, 
                # we perform the unfolding (window_size // focal_window_size) times with stride=1, then use masking 
                # operations to rule out the overlapped tokens, we find this does not necessarilly improve the performance 
                # but introduce more computational cost. As a result, we approximate it with the current implementation.     
                ...
                unfolded_mask = self.unfolds[k](mask.unsqueeze(0).unsqueeze(1)).view(
                    1, 1, self.focal_window, self.focal_window, -1).permute(0, 4, 2, 3, 1).contiguous().\
                ...
                (k_pooled_k, v_pooled_k) = map(
                    ...
                    view(-1, self.focal_window**2, self.num_heads, C // self.num_heads).transpose(1, 2), 
        ...
        window_area = (self.window_size[0] - 2) * (self.window_size[1] - 2)        
        ...
        relative_position_bias = relative_position_bias[None, None, None, :, :, :].repeat(topk_index_q.shape[:3] + (1,1,1))

        topk_index_q = topk_index_q.transpose(-2, -1)
        topk_index_kv = topk_index_kv.transpose(-2, -1)
        relative_position_bias_q = torch.gather(relative_position_bias, -2, topk_index_q.unsqueeze(-1).repeat(1, 1, 1, self.num_heads, 1, relative_position_bias.shape[-1]))
        relative_position_bias_qkv = torch.gather(relative_position_bias_q, -1, topk_index_kv.unsqueeze(-2).repeat(1, 1, 1, 1, relative_position_bias_q.shape[-2], 1))
        relative_position_bias_qkv = relative_position_bias_qkv.view(-1, self.num_heads, (self.window_size[0] - 2)*(self.window_size[1] - 2), (self.window_size[0] - 2)*(self.window_size[1] - 2))
        attn[:, :, :window_area, :window_area] = attn[:, :, :window_area, :window_area] + relative_position_bias_qkv

        # add relative position bias for patches inside a window
        if self.focal_level > 0:
            offset = window_area
            for k in range(self.focal_level):
                attention_bias_k = attention_bias[k].transpose(1, 2)[:, None, :, None, :].repeat(
                    1, nH * nW // (self.window_size[0]*self.window_size[1]), 1, 1, 1).view(
                        -1, self.num_heads, 1, self.topK
                    )
                attn[:, :, :window_area, offset:offset+self.topK] = attn[:, :, :window_area, offset:offset+self.topK] + attention_bias_k

                # add relative position bias
                # relative_position_index_k = getattr(self, 'relative_position_index_{}'.format(k))
                # relative_position_index_k_selected = relative_position_index_k[:, attention_index[k]].permute(3, 0, 1, 2).flatten(1)
                # relative_position_bias_to_windows = torch.gather(self.relative_position_bias_table_to_windows[k], 1, relative_position_index_k_selected).view(
                #     -1, self.nWh * self.nWw, B, self.topK,
                # ).permute(2, 1, 0, 3).contiguous().view(self.nWh * self.nWw * B, self.num_heads, 1, self.topK) # nH, NWh*NWw,input_resolution_h*input_resolution_w
                # attn[:, :, :window_area, offset:offset+self.topK] = attn[:, :, :window_area, offset:offset+self.topK] + relative_position_bias_to_windows
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(self.focal_level-1):
                ...
                relative_position_bias_to_windows = self.relative_position_bias_table_to_windows[k][:, relative_position_index_k.view(-1)].view(
                    -1, self.window_size[0] * self.window_size[1], self.focal_window**2,
                ) # nH, NWh*NWw,focal_region*focal_region
                attn[:, :, :window_area, offset:(offset + self.focal_window**2)] = \
                    attn[:, :, :window_area, offset:(offset + self.focal_window**2)] + relative_position_bias_to_windows.unsqueeze(0)
        ...
        x_selected = (attn @ v_all).transpose(1, 2).reshape(attn.shape[0], window_area, C)
        kv_windows_all = self.kv(x_windows).view(x_windows.shape[:-1] + (2, self.num_heads, C // self.num_heads))
        x_all = kv_windows_all[:, :, :, :, 1].view(-1, self.window_size[0]*self.window_size[1], C)
        # replace the selected query position with v from self-attention, otherwise keep the same
        x = torch.scatter(x_all, -2, topk_index_q.view(-1, topk_index_q.shape[-1], 1).repeat(1, 1, C), x_selected)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
