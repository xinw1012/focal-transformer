...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, window_size_glo, focal_window, 
        ...
        for k in range(self.focal_level):
            ...
            if k == 0:
                partition_window = 2*self.window_size[0]
            else:
                partition_window = self.focal_window

            relative_position_bias_table_to_windows = nn.Parameter(
                torch.zeros(
                    self.num_heads,
                    (2*partition_window - 1) * (2*partition_window - 1), 
                    )
            ) # 2*Wh-1 * 2*Ww-1, nH
            trunc_normal_(relative_position_bias_table_to_windows, std=.02)
            self.relative_position_bias_table_to_windows.append(relative_position_bias_table_to_windows)

            relative_position_index_k = get_relative_position_index(to_2tuple(partition_window), to_2tuple(partition_window))
            self.register_buffer("relative_position_index_{}".format(k), relative_position_index_k)
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):
            x_k = x_all[l_k]
            x_k_route_scores = self.routers[l_k](x_k)  # B, nH, nW, nHeads
            ...
            # partition feature map into macro-windows
            x_k_route_scores_window = window_partition_noreshape(x_k_route_scores, partition_window)
            x_k_route_scores_window = x_k_route_scores_window.view(
                B, x_k_route_scores_window.shape[1], x_k_route_scores_window.shape[2], 
                -1, self.num_heads
            )

            topK = min(x_k_route_scores_window.shape[3], self.topK // (2**l_k))        
            topK_levels.append(topK)

            topk_score_k, topk_index_k = torch.topk(x_k_route_scores_window, topK, dim=3) # B, topK, nHeads
            attention_bias.append(topk_score_k)
            attention_index.append(topk_index_k)

            if l_k == 0:
                k_k_window = window_partition_noreshape(k, partition_window)
                v_k_window = window_partition_noreshape(v, partition_window)
                k_k_window = k_k_window.view(B, k_k_window.shape[1], k_k_window.shape[2], -1, self.num_heads, C // self.num_heads)
                v_k_window = v_k_window.view(B, v_k_window.shape[1], v_k_window.shape[2], -1, self.num_heads, C // self.num_heads)
                k_k_selected = torch.gather(k_k_window, 3, topk_index_k.unsqueeze(-1).repeat(1, 1, 1, 1, 1, C // self.num_heads)).contiguous()
                v_k_selected = torch.gather(v_k_window, 3, topk_index_k.unsqueeze(-1).repeat(1, 1, 1, 1, 1, C // self.num_heads)).contiguous()
                k_k_selected = k_k_selected[:, :, None, :, None, :].repeat(1, 1, 2, 1, 2, 1, 1, 1).view(-1, topK, self.num_heads, C // self.num_heads).transpose(1, 2)
                v_k_selected = v_k_selected[:, :, None, :, None, :].repeat(1, 1, 2, 1, 2, 1, 1, 1).view(-1, topK, self.num_heads, C // self.num_heads).transpose(1, 2)                
            else:
                x_k_window = window_partition_noreshape(x_k, partition_window)
                x_k_window = x_k_window.view(B, x_k_window.shape[1], x_k_window.shape[2], -1, C)
                qkv_k_window = self.qkv(x_k_window).reshape(B, x_k_window.shape[1], x_k_window.shape[2], x_k_window.shape[3], 3, self.num_heads, C // self.num_heads)
                k_k_window, v_k_window = qkv_k_window[:,:,:,:,1], qkv_k_window[:,:,:,:,2]
                
                k_k_selected = torch.gather(k_k_window, 3, topk_index_k.unsqueeze(-1).repeat(1, 1, 1, 1, 1, C // self.num_heads)).contiguous()
                v_k_selected = torch.gather(v_k_window, 3, topk_index_k.unsqueeze(-1).repeat(1, 1, 1, 1, 1, C // self.num_heads)).contiguous()

                k_k_selected = k_k_selected[:, :, None, :, None, :].repeat(1, 1, partition_window, 1, partition_window, 1, 1, 1).view(-1, topK, self.num_heads, C // self.num_heads).transpose(1, 2)
                v_k_selected = v_k_selected[:, :, None, :, None, :].repeat(1, 1, partition_window, 1, partition_window, 1, 1, 1).view(-1, topK, self.num_heads, C // self.num_heads).transpose(1, 2)    

            k_all = torch.cat((k_all, k_k_selected), 2)
            v_all = torch.cat((v_all, v_k_selected), 2)
        ...
        if self.focal_level > 0:
            ...
            for l_k in range(self.focal_level):
                if l_k == 0:
                    attention_bias_k = attention_bias[l_k][:, :, None, :, None].transpose(-1, -2).repeat(
                        1, 1, 2, 1, 2, 1, 1).view(
                            -1, self.num_heads, 1, topK_levels[l_k]
                        )
                else:
                    attention_bias_k = attention_bias[l_k][:, :, None, :, None].transpose(-1, -2).repeat(
                        1, 1, self.focal_window, 1, self.focal_window, 1, 1).view(
                            -1, self.num_heads, 1, topK_levels[l_k]
                        )

                attn[:, :, :window_area, offset:offset+topK_levels[l_k]] = attn[:, :, :window_area, offset:offset+topK_levels[l_k]] + attention_bias_k
                
                # # add relative position bias
                relative_position_index_k = getattr(self, 'relative_position_index_{}'.format(l_k))
                relative_position_index_k_selected = relative_position_index_k[:, attention_index[l_k].view(-1)].view(
                    (-1,)+attention_index[l_k].shape).permute(5, 0, 1, 2, 3, 4).contiguous()
                relative_position_bias_to_windows = torch.gather(self.relative_position_bias_table_to_windows[l_k], 1, relative_position_index_k_selected.flatten(1))
                
                if l_k == 0:
                    relative_position_bias_to_windows = relative_position_bias_to_windows.view(relative_position_index_k_selected.shape).permute(2, 3, 4, 0, 1, 5).contiguous()
                    relative_position_bias_to_windows = relative_position_bias_to_windows.view(
                        B, relative_position_bias_to_windows.shape[1], relative_position_bias_to_windows.shape[2], self.num_heads, 2, self.window_size[0], 2, self.window_size[1], -1
                        ).permute(0, 1, 4, 2, 6, 3, 5, 7, 8).contiguous().view(-1, self.num_heads, window_area, topK_levels[l_k])
                else:
                    relative_position_bias_to_windows = relative_position_bias_to_windows.view(relative_position_index_k_selected.shape).permute(2, 3, 4, 1, 0, 5).contiguous()
                    relative_position_bias_to_windows = relative_position_bias_to_windows.unsqueeze(-2).view(-1, self.num_heads, 1, topK_levels[l_k])

                attn[:, :, :window_area, offset:offset+topK_levels[l_k]] = attn[:, :, :window_area, offset:offset+topK_levels[l_k]] + relative_position_bias_to_windows

                offset += topK_levels[l_k]
