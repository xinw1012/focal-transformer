...
def get_topk_closest_indice(q_windows, k_windows, topk=1):
    ...
    if relative_position_dists.shape[1] - factor**2 > 0 and factor > 1:
        topk = min(topk, relative_position_dists.shape[1]-factor**2)
        topk_score_k, topk_index_k = torch.topk(-relative_position_dists, topk + factor**2, dim=1) # B, topK, nHeads
        indice_topk = topk_index_k[:, factor**2:]
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, window_size_glo, focal_window, 
        ...
        for k in range(self.focal_level):
            ...
            # normalize coords into [-1,1]
            topk_closest_coord /= self.input_resolution[0]
            self.register_buffer("topk_cloest_indice_{}".format(k), topk_closest_indice)
            self.register_buffer("topk_cloest_coords_{}".format(k), topk_closest_coord)
    ...
    def forward(self, x_all, mask_all=None):
        ...
        k_all = k_windows; v_all = v_windows; topKs = []; topk_rpbs = []
        for l_k in range(self.focal_level):        
            ...
            topk_rpb_k = self.coord2rpb(topk_coords_k)
            ...
            k_all = torch.cat((k_all, k_k_selected.view(-1, self.num_heads, topk_closest_indice.shape[1], C // self.num_heads)), 2)
            v_all = torch.cat((v_all, v_k_selected.view(-1, self.num_heads, topk_closest_indice.shape[1], C // self.num_heads)), 2)
        ...
        topk_rpb_cat = torch.cat(topk_rpbs, 1).transpose(1, 2)[None, :, :, None, :].repeat(B, 1, 1, 1, 1).view(-1, self.num_heads, 1, window_area_whole-window_area)
        attn[:, :, :window_area, window_area:] = attn[:, :, :window_area, window_area:] + topk_rpb_cat
