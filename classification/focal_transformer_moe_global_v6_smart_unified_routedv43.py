...
class WindowAttention(nn.Module):
                ...
                if self.pool_method == "conv":
                    ...
                    self.pool_layers.append(
                        nn.Sequential(
                            ...
                            groups=dim, padding=kernel_size//2, padding_mode="zeros", bias=True),
                            # nn.BatchNorm2d(dim),
                            nn.GELU(),
                            )
                        )                                    
    ...
    def forward(self, x, mask_all=None):
        ...
        x4conv_kv_all = x4conv_kv # * self.gate(x4conv_kv.mean(dim=1, keepdim=True))
        if self.pool_method != "none": 
            # if we add coarser granularity and the pool method is not none
            for l in range(self.focal_level):         
                if self.pool_method in ["conv", "pool"]:
                    x4conv_kv = self.pool_layers[l](x4conv_kv)
                    x4conv_kv_gated = x4conv_kv * self.gate(x4conv_kv.mean(dim=1, keepdim=True))
                    x4conv_kv_all = x4conv_kv_all + x4conv_kv_gated
                elif self.pool_method == "conv_share":
        ...
        # x_global = self.avg_pool(x4conv_kv) # x4conv_kv.mean(2, keepdim=True).mean(3, keepdim=True)
        # x4conv_kv_all = x4conv_kv_all + x_global * self.gate(x_global.mean(dim=1, keepdim=True))        
        x4conv_kv_all = self.v(x4conv_kv_all)
        x_out = x4conv_q * x4conv_kv_all
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_conv_embed:
            ...
            x = x.view(B, H, W, C)
            # x = x.view(B, H//2, 2, W//2, 2, C).permute(0, 1, 3, 2, 4, 5).contiguous()
            # x = x.view(B, -1, 4 * C)
