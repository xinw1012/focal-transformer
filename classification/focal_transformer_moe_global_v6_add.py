...
def get_relative_position_distance(q_windows, k_windows, valid_range=1):
    ...
    relative_position_dists[torch.logical_and(relative_position_dists <= threshold,  relative_position_dists > 0)] = 0.0
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, window_size_glo, focal_window, 
        ...
        self.qkv = nn.Linear(dim, 2*self.num_heads + dim, bias=qkv_bias)
        ...
        self.pool_layers4qk = nn.ModuleList()
        self.pool_layers4v = nn.ModuleList()
        ...
        for k in range(self.focal_level):
            ...
            self.register_buffer("relative_position_range_table_to_windows_{}".format(k), relative_position_range_k_init)
            ...
            if k > 0:
                self.pool_layers4qk.append(nn.Conv2d(self.num_heads, self.num_heads, kernel_size=self.window_size, stride=self.window_size, groups=self.num_heads))
                self.pool_layers4v.append(nn.Conv2d(dim, dim, kernel_size=self.window_size, stride=self.window_size, groups=dim))
    ...
    def forward(self, x_all, mask_all=None):
        ...
        B, nH, nW, C = x.shape
        qkv = self.qkv(x).reshape(B, nH, nW, 2*self.num_heads+C)
        q, k, v = qkv[:,:,:,:self.num_heads], qkv[:,:,:,self.num_heads:2*self.num_heads], qkv[:,:,:,2*self.num_heads:]  # B, nH, nW, C

        # partition q map
        (q_windows, k_windows) = map(
            lambda t: window_partition(t, self.window_size[0]).view(
            -1, self.window_size[0] * self.window_size[0], self.num_heads).transpose(1, 2), 
            (q, k)
        )

        v_windows = window_partition(v, self.window_size[0]).view(
            -1, self.window_size[0] * self.window_size[0], self.num_heads, C // self.num_heads).transpose(1, 2)
        ...
        for l_k in range(self.focal_level):    
            if l_k == 0:    
                k_k = k.view(B, -1, self.num_heads)
                v_k = v.view(B, -1, self.num_heads, C // self.num_heads)                
                x_k_route_scores_k = k_k
            else:
                k_k = self.pool_layers4qk[l_k-1](k.permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
                v_k = self.pool_layers4v[l_k-1](v.permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
                k_k = k_k.view(B, -1, self.num_heads)
                v_k = v_k.view(B, -1, self.num_heads, C // self.num_heads)
                x_k_route_scores_k = k_k
            ...
            relative_position_range_k = getattr(self, "relative_position_range_table_to_windows_{}".format(l_k))
            # relative_position_range_k = self.relative_position_range_table_to_windows[l_k]
            relative_position_bias_table_to_windows_k = relative_position_range_k.unsqueeze(-1)

            if l_k == 0:
                relative_position_bias_table_to_windows_k = relative_position_bias_table_to_windows_k.view(
                    relative_position_bias_table_to_windows_k.shape[0], self.nWh, 1, self.nWw, 1, 1)                
                ...
                relative_position_bias_table_to_windows_k = relative_position_bias_table_to_windows_k.view(relative_position_bias_table_to_windows_k.shape[0], -1, 1)                
            ...
            k_k_selected = torch.gather(k_k, 1, topk_index_k.view(B, -1, self.num_heads))
            v_k_selected = torch.gather(v_k, 1, topk_index_k.view(B, -1, self.num_heads).unsqueeze(-1).repeat(1, 1, 1, C // self.num_heads))

            k_k_selected = k_k_selected.view((-1,) + topk_index_k.shape[2:])
            v_k_selected = v_k_selected.view((-1,) + topk_index_k.shape[2:] + (C // self.num_heads,)).transpose(1, 2)

            k_all = torch.cat((k_all, k_k_selected.transpose(1, 2)), 2)
            v_all = torch.cat((v_all, v_k_selected.view(-1, self.num_heads, topK, C // self.num_heads)), 2)
        
        self.route_scores = x_k_route_scores_all

        attn_base = (q_windows.transpose(1, 2) @ k_all).unsqueeze(1) * (self.num_heads ** -0.5)
        N = k_all.shape[-2]
        attn = (attn_base + k_all.unsqueeze(-2))  # B*nW, nHead, window_size*window_size, focal_window_size*focal_window_size
        ...
        if self.focal_level > 0:
            ...
            for k in range(self.focal_level):
                ...
                relative_position_bias_to_windows = torch.gather(self.relative_position_bias_table_to_windows[k], 0, 
                    relative_position_index_k_selected.view(-1, 1)).view(relative_position_index_k_selected.shape)
