...
class WindowAttention(nn.Module):
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):        
            ...
            rte_scores_all.append(rte_score_k_selected)

            k_k_selected = torch.gather(k_k, 1, topk_indice_k.view(B, -1, 1).unsqueeze(-1).repeat(1, 1, self.num_heads, C // self.num_heads)).view(
                B, *topk_closest_indice.shape, self.num_heads, C // self.num_heads).transpose(2, 3)
            v_k_selected = torch.gather(v_k, 1, topk_indice_k.view(B, -1, 1).unsqueeze(-1).repeat(1, 1, self.num_heads, C // self.num_heads)).view(
                B, *topk_closest_indice.shape, self.num_heads, C // self.num_heads).transpose(2, 3)
        ...
        topk_rpbs_all = torch.cat(topk_rpbs, 2)
        rte_scores_all = torch.cat(rte_scores_all, 2)
        rte_scores_all = rte_scores_all.view(-1, *rte_scores_all.shape[2:]).transpose(1, 2)

        if self.use_route:
            topk_scores, topk_indice = torch.topk(rte_scores_all, min(64, rte_scores_all.shape[2]), dim=2)
            k_all_routed = torch.gather(k_all, 2, topk_indice.unsqueeze(-1).repeat(1, 1, 1, k_all.shape[-1]))
            v_all_routed = torch.gather(v_all, 2, topk_indice.unsqueeze(-1).repeat(1, 1, 1, k_all.shape[-1]))
            topk_rpbs_routed = torch.gather(
                topk_rpbs_all.unsqueeze(0).repeat(B, 1, 1, 1, 1).view(-1, *topk_rpbs_all.shape[1:]), 
                2, topk_indice.unsqueeze(1).transpose(-1, -2).repeat(1, q_windows.shape[2], 1, 1)
            )
            topk_scores_routed = topk_scores.unsqueeze(-2)
        else:
            k_all_routed = k_all
            v_all_routed = v_all
            topk_rpbs_routed = topk_rpbs_all.unsqueeze(0).repeat(B, 1, 1, 1, 1).view(-1, *topk_rpbs_all.shape[1:])        
        ...
        attn = attn + topk_rpbs_routed.permute(0, 3, 1, 2).contiguous() + topk_scores_routed
...
class FocalTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, expand_size=0, shift_size=0,
        ...
        self.attn = WindowAttention(
            ...
            pool_method=pool_method, use_route=use_route, topK=topK)
