    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):        
            ...
            if self.use_route:
                ...
                # selecting top tokens also considering the relative position
                rte_score_k_selected = rte_score_k_selected + topk_rpb_k.unsqueeze(0).repeat(B, 1, 1, 1, 1).mean(2)

                # select a subset of tokens according to routing scores
                topk_rte_score_k, topk_rte_indice_k = torch.topk(
                    rte_score_k_selected, min(int(self.window_size[0]*self.window_size[1]), rte_score_k_selected.shape[2]), dim=2)
