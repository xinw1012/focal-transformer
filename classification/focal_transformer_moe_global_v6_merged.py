...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, window_size_glo, focal_window, 
        ...
        self.pool_layers4qkv = nn.ModuleList()
        ...
        for k in range(self.focal_level):
            ...
            if k > 0:
                self.pool_layers4qkv.append(nn.Conv2d(dim, dim, kernel_size=self.window_size, stride=self.window_size, groups=dim))
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):        
            if l_k == 0:
                ...
                x_k_route_scores_k = self.routers[l_k](k_k.view(B, -1, C))
            else:
                k_k = self.pool_layers4qkv[l_k-1](k.permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
                v_k = self.pool_layers4qkv[l_k-1](v.permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
                k_k = k_k.view(B, -1, self.num_heads, C // self.num_heads)
                v_k = v_k.view(B, -1, self.num_heads, C // self.num_heads)
                x_k_route_scores_k = self.routers[l_k](k_k.view(B, -1, C))

            if l_k == 0:
                x_k_route_scores_all = x_k_route_scores_k.view(B, nH, nW, 1)
