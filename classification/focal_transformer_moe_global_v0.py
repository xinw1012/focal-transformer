...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, window_size_glo, focal_window, 
        ...
        for k in range(self.focal_level):
            ...
            if k == 0:
                map_size = self.input_resolution
            elif k == 1:
                map_size = (self.input_resolution[0] // self.window_size[0], self.input_resolution[1] // self.window_size[1])
            
            relative_position_bias_table_to_windows = nn.Parameter(
                torch.zeros(
                    self.num_heads,
                    (self.nWh + map_size[0] - 1) * (self.nWw + map_size[1] - 1), 
                    )
            ) # 2*Wh-1 * 2*Ww-1, nH
            trunc_normal_(relative_position_bias_table_to_windows, std=.02)
            self.relative_position_bias_table_to_windows.append(relative_position_bias_table_to_windows)

            relative_position_index_k = get_relative_position_index((self.nWh, self.nWw), map_size)
            self.register_buffer("relative_position_index_{}".format(k), relative_position_index_k)
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):
            ...
            relative_position_bias_table_to_windows_k = \
                self.relative_position_bias_table_to_windows[l_k].transpose(0, 
                    1)[relative_position_index_k.view(-1), :].view(relative_position_index_k.shape + (self.num_heads,))

            x_k_route_scores = x_k_route_scores.unsqueeze(1) + relative_position_bias_table_to_windows_k.unsqueeze(0)

            topK = min(self.topK, x_k_route_scores.shape[2]); topKs.append(topK)

            topk_score_k, topk_index_k = torch.topk(x_k_route_scores, topK, dim=2) # B, topK, nHeads
