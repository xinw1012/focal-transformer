...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, focal_factor, 
        ...
        self.linear = nn.Linear(dim + self.focal_level * (dim // 4), dim, bias=qkv_bias)
        # self.conv = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1, groups=dim, bias=False)
        # self.gate = nn.Linear(dim, 1, bias=qkv_bias)
        ...
        if self.pool_method != "none":
            for k in range(self.focal_level):
                ...
                if self.pool_method == "conv":
                    ...
                    self.linear_layers.append(nn.Linear(dim, dim // 4, bias=qkv_bias)) 
    ...
    def forward(self, x_all, mask_all=None):
        ...
        B, nH, nW, C = x.shape
        # x = self.se_block(x)
        x_outs = [x - x.mean(1, keepdim=True).mean(2, keepdim=True).expand(-1, nH, nW, -1)]
        ...
        if self.pool_method != "none": 
            ...
            for l in range(self.focal_level):         
                ...
                x_outs.append(self.linear_layers[l](self.act(x_l)))
