...
class TokenAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., 
        focal_level=1, focal_window=7, focal_factor=2):
        ...
        # define a parameter table of relative position bias
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros(num_heads, window_size[0]*window_size[1]))  # 2*Wh-1 * 2*Ww-1, nH
        ...
        self.unfold = nn.Unfold(window_size, padding=window_size[0] // 2, stride=1)
    ...
    def forward(self, x, mask_all=None):
        ...
        qkv = self.qkv(x)
        q, kv = torch.split(qkv, (C, 2*C), -1)

        kv4unfold = kv.permute(0, 3, 1, 2).contiguous()    
        kv_unfolded = self.unfold(kv4unfold).view(B, 2*C, *self.window_size, nH, nW)
        k, v = torch.split(kv_unfolded, (C, C), 1)

        q = q.reshape(B, nH*nW, self.num_heads, 1, C // self.num_heads)
        k = k.permute(0, 4, 5, 2, 3, 1).contiguous().reshape(B, nH*nW, -1, self.num_heads, C // self.num_heads).transpose(2, 3)
        v = v.permute(0, 4, 5, 2, 3, 1).contiguous().reshape(B, nH*nW, -1, self.num_heads, C // self.num_heads).transpose(2, 3)
        q = q * self.scale

        attn = (q @ k.transpose(-2, -1))

        relative_position_bias = self.relative_position_bias_table[None, None, :, None, :]  # nH, Wh*Ww, Wh*Ww
        attn = attn + relative_position_bias
        attn = self.softmax(attn)
        attn = self.attn_drop(attn)

        x = (attn @ v).view(B, nH, nW, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
    ...
    def flops(self, N):
        ...
        flops += N * self.dim * 3 * self.dim
        # attn = (q @ k.transpose(-2, -1))
        flops += self.num_heads * N * (self.dim // self.num_heads) * self.window_size[0] * self.window_size[1]
        #  x = (attn @ v)
        flops += self.num_heads * N * self.window_size[0] * self.window_size[1] * (self.dim // self.num_heads)
        # x = self.proj(x)
        flops += N * self.dim * self.dim
        return flops
...
class FocalConvolution(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, focal_factor, 
        ...
        if self.pool_method != "none":
            for k in range(self.focal_level):
                kernel_size = min(self.focal_factor*k+self.focal_window, min(input_resolution))
    ...
    def forward(self, x, mask_all=None):
        ...
        """
        B, nH, nW, C = x.shape
        x4conv = x.permute(0, 3, 1, 2).contiguous()
        ...
        if self.pool_method != "none": 
            ...
            for l in range(self.focal_level):         
                if self.pool_method in ["conv", "pool"]:
                    ...
                    x4conv = self.pool_layers[l](x4conv)
                    x_all_levels.append(x4conv)

        x_global = self.act(x4conv.mean(2, keepdim=True).mean(3, keepdim=True)).repeat(1, 1, nH, nW)
        x_all_levels.append(x_global)

        context_all_levels = torch.stack(x_all_levels, 1).permute(0, 1, 3, 4, 2).contiguous()
        
        q = self.q(x)
        q = q.view(B, 1, nH, nW, self.num_heads, C // self.num_heads)
        q = q * self.scale

        kv = self.kv(context_all_levels)
        k, v = torch.split(kv, (C, C), -1)
        k = k.view(B, self.focal_level+2, nH, nW, self.num_heads, C // self.num_heads)
        v = v.view(B, self.focal_level+2, nH, nW, self.num_heads, C // self.num_heads)

        attn = (q * k).sum(-1, keepdim=True)
        attn = self.softmax(attn)
        attn = self.attn_drop(attn)
        x_out = (attn * v).sum(1).view(B, nH, nW, C)
...
class FocalTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, expand_size=0, shift_size=0,
        ...
        self.attn = TokenAttention(
            ...
            focal_window=focal_window, focal_factor=focal_factor, focal_level=self.focal_level)
