...
class WindowAttention(nn.Module):
            ...
            for k in range(self.focal_level):
                ...
                if self.pool_method == "conv":
                    ...
                    self.pool_layers.append(
                        nn.Sequential(
                            nn.Conv2d(head_dim, head_dim, kernel_size=kernel_size, stride=self.focal_stride, 
                            groups=head_dim, padding=kernel_size//2, padding_mode="zeros", bias=False),
                            # nn.BatchNorm2d(dim),
                            nn.GELU(),
                            )
                        )              
    ...
    def forward(self, x_all, mask_all=None):
        ...
        x4conv_kv = x.reshape(-1, self.head_dim, nH, nW)
        ...
        if self.pool_method != "none": 
            ...
            for l in range(self.focal_level):         
                if self.pool_method in ["conv", "pool"]:
                    ...
                    modulator = modulator + x4conv_kv.view(B, C, nH, nW)*self.focal_weights[l]
        x4conv_kv = x4conv_kv.view(B, C, nH, nW)                    
        # x_global = self.act(x4conv_kv.mean(2, keepdim=True).mean(3, keepdim=True))
        # modulator = modulator + x_global*self.focal_weights[-1]
        modulator = self.gate(modulator) 
        modulator = modulator.permute(0, 2, 3, 1).contiguous()
        x4conv_kv = x4conv_kv.permute(0, 2, 3, 1).contiguous()

        return modulator, x4conv_kv
...
class FocalTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, expand_size=0, shift_size=0,
        ...
        self.dw = nn.Conv2d(dim, dim, kernel_size=7, groups=dim, padding=3)
        self.act = act_layer()
    ...
    def forward(self, x):
        ...
        modulator = self.act(self.dw(x).view(B, C, L).transpose(1, 2))

        # FFN
        x = shortcut * modulator.mean(1, keepdim=True) + self.drop_path(self.gamma_1 * modulator.mean(2, keepdim=True))
...
if __name__ == '__main__':
    ...
    model = FocalTransformer(
        img_size=img_size, 
        embed_dim=96, 
        depths=[3,3,9,3], drop_path_rate=0.5, 
        focal_levels=[1,1,1,1], 
        focal_factors=[2,2,2,2], 
        focal_strides=[1,1,1,1], 
