...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, 
        ...
        self.attn = nn.Linear(head_dim, 1, bias=qkv_bias)
    ...
    def forward(self, x_all, mask_all=None):
        ...
        x_outs = self.linear(x_outs)

        x_outs = x_outs.view(B, nH, nW, -1, self.num_heads, C // self.num_heads).transpose(-3, -2)
        x_outs = x_outs * self.scale

        attn = self.attn(x_outs).transpose(-2, -1)
        attn = self.softmax(attn)
        attn = self.attn_drop(attn)

        x_out = (attn @ x_outs).view(B, nH, nW, C)
