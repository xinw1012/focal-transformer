...
class WindowAttention(nn.Module):
    ...
    def forward(self, x_all, mask_all=None):
        ...
        # select a subset of k_all
        routing_scores = self.router(k_all)
        if routing_scores.shape[2] < 49:
            k_all_routed = k_all
            v_all_routed = v_all
            topk_rpb_cat = torch.cat(topk_rpbs, 2).permute(0, 3, 1, 2).contiguous().unsqueeze(0).repeat(B, 1, 1, 1, 1)
            topk_rpb_cat = topk_rpb_cat.view(-1, *topk_rpb_cat.shape[2:])
            topk_rpb_cat_routed = topk_rpb_cat         
        else:
            # 128 + 64
            topk = 49
            topk_scores, topk_indice = torch.topk(routing_scores, topk, dim=2)

            # gather topk key and value
            (k_all_routed, v_all_routed) = map(
                lambda t: torch.gather(t, 2, topk_indice.repeat(1, 1, 1, C // self.num_heads)), (k_all, v_all)
            )            
            # also gather the rpb
            topk_rpb_cat = torch.cat(topk_rpbs, 2).permute(0, 3, 1, 2).contiguous().unsqueeze(0).repeat(B, 1, 1, 1, 1)
            topk_rpb_cat = topk_rpb_cat.view(-1, *topk_rpb_cat.shape[2:])
            topk_rpb_cat_routed = torch.gather(topk_rpb_cat, -1, topk_indice.transpose(-1, -2).repeat(1, 1, topk_rpb_cat.shape[2], 1))

        attn = (q_windows @ k_all_routed.transpose(-2, -1))  # B*nW, nHead, window_size*window_size, focal_window_size*focal_window_size
        attn = attn + topk_rpb_cat_routed
...
class FocalTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, expand_size=0, shift_size=0,
        ...
        self.attn = WindowAttention(
            ...
            pool_method=pool_method, conv_route=conv_route, topK=topK)
