...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, 
        ...
        self.linear = nn.Linear(dim*2, dim, bias=qkv_bias)
        ...
        self.pool_layer = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=3, stride=1, groups=dim, padding=1),
            # nn.ReLU(),
        )                    
    ...
    def forward(self, x_all, mask_all=None):
        ...
        x_out = x_outs[-1]
        for l in range(self.focal_level):
            x_out = torch.cat((x_out, x_outs[self.focal_level-l-1]), -1)
