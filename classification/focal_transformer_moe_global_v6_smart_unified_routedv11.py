...
# changelog compared with v10
# instead of using network to embed relative position bias, we still use lookup table as in swin and focal
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, 
                    ...
                    use_route=False, topK=64, routing_topK=64, shape="circle", dilation=1, offset=0):
        ...
        if self.pool_method != "none":
            for k in range(self.focal_level-1):
                if self.pool_method == "conv":
                    self.pool_layers.append(nn.Conv2d(dim, dim, kernel_size=window_size[0]+4, stride=window_size[0], groups=dim, padding=2))
                elif self.pool_method == "conv_share":
                    # share parameters across different channels     
                    if k == 0:
                        self.pool_layers.append(
                            nn.Sequential(
                                nn.Conv2d(1, 1, kernel_size=window_size[0]+4, stride=window_size[0], groups=1, padding=2),
                                nn.GELU(),
                            )
                        )                             
                    else:
                        self.pool_layers.append(
                            nn.Sequential(
                                nn.Conv2d(1, 1, kernel_size=3, stride=1, groups=1, padding=1),
                                nn.GELU(),
                            )
                        )   
            self.focal_bias = nn.Parameter(torch.zeros(window_size[0]*window_size[1], self.focal_level-1))
        ...
        for k in range(1):
            ...
            focal_size = (
                int(torch.max(topk_closest_coord_window[:,:,:,0].max().abs(), topk_closest_coord_window[:,:,:,0].min().abs()).item()) + 1, 
                int(torch.max(topk_closest_coord_window[:,:,:,1].max().abs(), topk_closest_coord_window[:,:,:,1].min().abs()).item()) + 1,
            )
            self.focal_size = focal_size
            # define a parameter table of relative position bias
            self.relative_position_bias_table = nn.Parameter(
                torch.zeros((2 * focal_size[0] - 1) * (2 * focal_size[1] - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(1):        
            ...
            a = topk_coords_k[:,:,:,0] + self.focal_size[0] - 1
            a = a * (2 * self.focal_size[0] - 1)
            b = topk_coords_k[:,:,:,1] + self.focal_size[1] - 1
            inds = (a + b).long()
            topk_rpb_k = self.relative_position_bias_table[inds.flatten(0)].view(*inds.shape, -1)
        ...
        (k_p, v_p) = map(
            lambda t: t.permute(0, 3, 1, 2).contiguous().view(B * C, 1, nH, nW), (k, v)
        )

        kv_p = torch.cat((k_p, v_p), 0)
        ...
        if self.focal_level > 1 and self.pool_method != "none": 
            ...
            for k in range(self.focal_level-1):         
                if self.pool_method == "conv":
                    ...
                    kv_p = self.pool_layers[k](kv_p)

                    k_windows_pooled = kv_p[:B*C].view(B, C, *kv_p.shape[2:]).permute(0, 2, 3, 1).contiguous() # B, nWh, nWw, C     
                    k_windows_pooled = k_windows_pooled.view(-1, self.num_heads, 1, C // self.num_heads)
                    k_all.append(k_windows_pooled)

                    v_windows_pooled = kv_p[B*C:].view(B, C, *kv_p.shape[2:]).permute(0, 2, 3, 1).contiguous() # B, nWh, nWw, C     
                    v_windows_pooled = v_windows_pooled.view(-1, self.num_heads, 1, C // self.num_heads)
                    v_all.append(v_windows_pooled)
        ...
        if self.focal_level > 1:
            attn[:,:,:,:-(self.focal_level-1)] = attn[:,:,:,:-(self.focal_level-1)] + topk_rpbs_routed.permute(0, 3, 1, 2)
            attn[:,:,:,-(self.focal_level-1):] = attn[:,:,:,-(self.focal_level-1):] + self.focal_bias.unsqueeze(0).unsqueeze(0)
        else:
...
class FocalTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, expand_size=0, shift_size=0,
                 ...
                 focal_level=1, focal_window=1, topK=64, routing_topK=64, shape="circle", dilation=1, offset=0,
        ...
        self.attn = WindowAttention(
            ...
            shape=shape, dilation=dilation, offset=offset)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, expand_size, expand_layer,
                 ...
                 use_shift=False, use_pre_norm=False, dilation=1, shape="circle", 
        ...
        if downsample is not None:
            self.downsample = downsample(
                ...
                use_conv_embed=self.use_conv_embed, norm_layer=norm_layer, use_pre_norm=use_pre_norm, 
                is_stem=False
            )
            if self.use_conv_embed:
                self.conv = nn.Conv2d(2*dim, 2*dim, kernel_size=3, padding=1, stride=1, groups=2*dim)
        else:
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            ...
            x = self.downsample(x)
            if self.use_conv_embed:
                x = x + self.conv(x.view(x.shape[0], self.input_resolution[0] // 2, self.input_resolution[1] // 2, -1).permute(0, 3, 1, 2).contiguous()
                    ).permute(0, 2, 3, 1).contiguous().view(x.shape)
        return x
...
if __name__ == '__main__':
    ...
    model = FocalTransformer(
        img_size=img_size, 
        embed_dim=192, 
        depths=[1,3,18,2], drop_path_rate=0.2, 
        # focal_levels=[1,1,1,1], 
        focal_levels=[5,4,3,2], 
