...
def get_topk_closest_indice(input_resolution, window_size_q, window_size_k, topk=1):
    # get pair-wise relative position index for each token inside the window
    q_windows = (
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, 
                    ...
                    use_route=False, topK=64, routing_topK=64):
        ...
        for k in range(self.focal_level):
            ...
            topk_closest_indice, topk_closest_coord = get_topk_closest_indice(
                input_resolution, self.window_size, (1, 1) if k == 0 else (self.focal_window, self.focal_window), self.topK // (2**k))
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):        
            ...
            if self.use_route:
                ...
                rte_scores_k_selected_positioned = \
                    rte_score_k_selected.view(-1, 1, *rte_score_k_selected.shape[2:]) + topk_rpb_k
                import pdb; pdb.set_trace()

                # select a subset of tokens according to routing scores
                topk_rte_score_k, topk_rte_indice_k = torch.topk(
                    rte_scores_k_selected_positioned, min(int(self.window_size[0]*self.window_size[1]), rte_score_k_selected.shape[2]), dim=2)
                rte_scores_all.append(topk_rte_score_k)

                import pdb; pdb.set_trace()

                topk_indice_k_routed = torch.gather(
                    topk_closest_indice.unsqueeze(0).unsqueeze(2).unsqueeze(-1).repeat(B, 1, topk_rte_indice_k.shape[1], 1, self.num_heads).view(
                        -1, topk_rte_indice_k.shape[1], topk_closest_indice.shape[1], self.num_heads), 
                    2,
                    topk_rte_indice_k
                )

                import pdb; pdb.set_trace()
                topk_rpb_k = torch.gather(topk_rpb_k.unsqueeze(0).repeat(B, 1, 1, 1, 1).view(-1, *topk_rpb_k.shape[1:]), 2, topk_rte_indice_k)
            else:
            ...
            k_k_selected = torch.gather(k_k, 1, topk_indice_k_routed.view(B, -1, self.num_heads).unsqueeze(-1).repeat(1, 1, 1, C // self.num_heads)).view(
                B, *topk_indice_k_routed.shape[0:3], self.num_heads, C // self.num_heads).transpose(3, 4)
