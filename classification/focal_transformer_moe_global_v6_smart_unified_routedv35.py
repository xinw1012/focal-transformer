...
class WindowAttention(nn.Module):
    ...
    def forward(self, x_all, mask_all=None):
        ...
        if nH >= 28 and nW >= 28:
            x_pooled = self.pool_global(x.permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
            x_pooled = x_pooled[:, :, None, :, None, :].expand(-1, -1, 14, -1, 14, -1).contiguous().view(x.shape)
            x = x * x_pooled
            x_outs = [x]
        else:
            x = x * x_avg
