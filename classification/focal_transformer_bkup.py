...
class WindowAttention(nn.Module):
    ...
    def forward(self, x_all, mask_all=None):
        ...
        if self.expand_size > 0:
            ...
            (k_tl, v_tl) = map(
                lambda t: torch.roll(t, shifts=(-self.expand_size, -self.expand_size), dims=(1, 2)),
                (k, v)
            )
            ...
            (k_tl_windows, k_tr_windows, k_bl_windows, k_br_windows) = map(
                lambda t: window_partition(t, self.window_size[0]).view(-1, self.window_size[0] * self.window_size[0], self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous(), 
                (k_tl, k_tr, k_bl, k_br)
            )            
            ...
            k_rolled = torch.cat((k_tl_windows, k_tr_windows, k_bl_windows, k_br_windows), 2)
            v_rolled = torch.cat((v_tl_windows, v_tr_windows, v_bl_windows, v_br_windows), 2)
        ...
        if self.pool_method != "none":
            x_pooled = torch.cat(x_all[1:], 1)
            B_p, nW_p, C_p = x_pooled.shape
            qkv_pooled = self.qkv(x_pooled).reshape(B_p, nW_p, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4).contiguous()
            q_pooled, k_pooled, v_pooled = qkv_pooled[0], qkv_pooled[1], qkv_pooled[2]  # B, nH, nW, C
        ...
        if self.pool_method != "none":
            ...
            for k in range(self.focal_level):
                ...
                attn[:, :, :window_area, offset:(offset + x_all[k+1].shape[1])] = \
                    attn[:, :, :window_area, offset:(offset + x_all[k+1].shape[1])] + relative_position_bias_xwin.unsqueeze(0)
                # add mask
                attn[:, :, :window_area, offset:(offset + x_all[k+1].shape[1])] = \
                    attn[:, :, :window_area, offset:(offset + x_all[k+1].shape[1])] + \
                        mask_all[k+1][:, :, None, None, :].repeat(attn.shape[0] // mask_all[k+1].shape[1], 1, 1, 1, 1).view(-1, 1, 1, mask_all[k+1].shape[-1])
                offset += x_all[k+1].shape[1]
    ...
    def forward(self, x):
        ...
        if self.pool_method != "none":
            for k in range(self.focal_level):                
                ...
                # print("time cost for partition: ", time.time() - tic)
                # tic = time.time()
                ...
                # print("time cost for fc: ", time.time() - tic)
                # tic = time.time()
                ...
                # print("time cost for unfold x: ", time.time() - tic)
                # tic = time.time()
                ...
                # print("time cost for create mask: ", time.time() - tic)
                # tic = time.time()
                ...
                # print("time cost for unfold mask: ", time.time() - tic)
                # tic = time.time()      
                ...
                # print("time cost for fill mask: ", time.time() - tic)
                # tic = time.time()                
                x_window_masks_all += [x_window_masks]
                # print("time cost for unfold mask: ", time.time() - tic)
                # tic = time.time()

        # print("time cost for pool: ", time.time() - tic)
        # tic = time.time()
        ...
        # print("time cost for sa: ", time.time() - tic)
        # import pdb; pdb.set_trace()
...
if __name__ == '__main__':
    ...
    model = FocalTransformer(embed_dim=96, depths=[2,2,6,2], drop_path_rate=0.2, 
        focal_levels=[1,1,1,1], num_heads=[3,6,12,24]).cuda()
    flops = model.flops()
