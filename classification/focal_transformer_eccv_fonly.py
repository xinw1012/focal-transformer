...
class FocalConvolution(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, focal_factor, 
                    focal_level, num_heads, mlp_ratio=4.0, focal_stride=1, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., pool_method="none", 
        ...
        self.pre_conv = nn.Linear(dim, dim + mlp_hidden_dim + (self.focal_level+1), bias=qkv_bias)
        # self.v = nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0, groups=1, bias=qkv_bias)
        self.gate = nn.Sequential(
            nn.Conv2d(mlp_hidden_dim, dim, kernel_size=1, stride=1, padding=0, groups=1, bias=qkv_bias), 
        )
                ...
                if self.pool_method == "conv":
                    ...
                    if kernel_size <= min(input_resolution):       
                        self.pool_layers.append(
                            nn.Sequential(
                                nn.Conv2d(mlp_hidden_dim, mlp_hidden_dim, kernel_size=kernel_size, stride=self.focal_stride, 
                                groups=mlp_hidden_dim, padding=kernel_size//2, padding_mode="zeros", bias=False),
                                # nn.BatchNorm2d(dim),
                                nn.GELU(),
                                )
                            )              
    ...
    def forward(self, x, mask_all=None):
        ...
        x4conv_q, x4conv_kv, focal_weights = torch.split(x, (C, self.mlp_hidden_dim, self.focal_level+1), 1)
        # HxWxd, HxWxd, HxWx(l+1)
        # offset = 0
    ...
    def flops(self, N):
        ...
        flops += N * self.dim * (self.dim + self.mlp_hidden_dim + (self.focal_level+1))

        # focal convolution
        for k in range(self.focal_level):
            flops += N * (self.kernel_sizes[k]**2+1) * self.mlp_hidden_dim
        
        # global gating
        flops += N * 1 * self.mlp_hidden_dim

        # linear project plus element-wise mul
        flops += N * self.mlp_hidden_dim * (self.dim + 1)
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, expand_size=0, shift_size=0,
        ...
        if self.input_resolution[0] < 7:        
            ...
            self.attn = FocalConvolution(
                ...
                focal_level=self.focal_level, num_heads=num_heads, mlp_ratio=mlp_ratio, 
                qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, 
...
if __name__ == '__main__':
    ...
    model = FocalTransformer(
        img_size=img_size, 
        embed_dim=96, 
        depths=[6,6,18,6], drop_path_rate=0.5, 
        focal_levels=[1,1,1,1], 
        focal_factors=[2,2,2,2], 
        focal_strides=[1,1,1,1], 
