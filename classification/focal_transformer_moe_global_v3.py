...
# make relative position range bias learnable, instead of fixed
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):        
            ...
            # if x_k_route_scores_positioned.max() < -1000:
            #     x_k_route_scores_positioned = x_k_route_scores_positioned + 100000.0
            ...
            k_k_selected = k_k_selected.view((B,) + topk_index_k.shape[1:3] + (self.num_heads, C // self.num_heads,)).transpose(2, 3)
            v_k_selected = v_k_selected.view((B,) + topk_index_k.shape[1:3] + (self.num_heads, C // self.num_heads,)).transpose(2, 3)
...
if __name__ == '__main__':
    ...
    model = FocalTransformer(img_size=img_size, embed_dim=96, depths=[1,3,6,2], drop_path_rate=0.2, 
