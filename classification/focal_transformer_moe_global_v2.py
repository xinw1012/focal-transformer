...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, window_size_glo, focal_window, 
        ...
        self.relative_position_temp_table_to_windows = nn.ParameterList()
        self.relative_position_bias_table_to_windows = nn.ParameterList()

        for k in range(self.focal_level):
            # build relative position bias
            relative_position_bias_k = get_relative_position_distance(
                (self.nWh, self.nWw), (self.nWh, self.nWw), 
                valid_range=(1 if k==0 else 100))
            self.register_buffer("relative_position_bias_{}".format(k), relative_position_bias_k)

            # relative_position_temp_table_to_windows = nn.Parameter(
            #     torch.ones(self.num_heads)
            # )
            # self.relative_position_temp_table_to_windows.append(relative_position_temp_table_to_windows)
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):        
            ...
            relative_position_bias_k = getattr(self, "relative_position_bias_{}".format(l_k))
            # relative_position_temp_table_to_windows_k = self.relative_position_temp_table_to_windows[l_k]

            # relative_position_bias_table_to_windows_k = torch.exp(-relative_position_bias_k.unsqueeze(-1) / relative_position_temp_table_to_windows_k.view(1, 1, -1))
            # relative_position_bias_table_to_windows_k[relative_position_bias_k.unsqueeze(-1).repeat(1, 1, self.num_heads)==100000].fill_(-100000)
            relative_position_bias_table_to_windows_k = relative_position_bias_k.unsqueeze(-1).repeat(1, 1, self.num_heads)
