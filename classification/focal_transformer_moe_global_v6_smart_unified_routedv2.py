...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, window_size_glo, focal_window, 
        ...
        for k in range(self.focal_level):
            ...
            topk_closest_indice, topk_closest_coord = get_topk_closest_indice(
                (self.nWh, self.nWw), (range_h, range_w), self.topK if k == 0 else 36)
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):        
            ...
            if self.use_route:
                ...
                topk_rte_score_k, topk_rte_indice_k = torch.topk(
                    rte_score_k_selected, min(48, rte_score_k_selected.shape[2]), dim=2)
        ...
        (k_all, v_all, topk_rpbs_all, rte_scores_all) = map(
            lambda t: torch.cat(t, 2), (k_all, v_all, topk_rpbs, rte_scores_all)
        )
        ...
        attn = attn + topk_rpbs_routed.permute(0, 3, 1, 2) + topk_scores_routed
...
if __name__ == '__main__':
    ...
    model = FocalTransformer(
        img_size=img_size, 
        embed_dim=64, 
        depths=[1,2,21,1], drop_path_rate=0.2, 
        focal_levels=[2,2,2,2], expand_sizes=[3,3,3,3], 
        expand_layer="all", 
        focal_windows=[7,5,3,1], 
        num_heads=[2,4,8,16],
        focal_topK=96, 
