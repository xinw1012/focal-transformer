...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, focal_factor, 
        ...
        self.pre_conv = nn.Linear(dim, 3*dim, bias=qkv_bias)
        # self.pre_conv = nn.Conv2d(dim, 2*dim, kernel_size=1, stride=1, padding=0, groups=1, bias=qkv_bias)
    ...
    def forward(self, x_all, mask_all=None):
        ...
        x4conv_q, x4conv_kv = torch.split(x, (C, 2*C), 1)
        x4conv_kv = x4conv_kv.reshape(B*2, C, nH, nW)

        x4conv_kv_all = [x4conv_kv]
        if self.pool_method != "none": 
            # if we add coarser granularity and the pool method is not none
            for l in range(self.focal_level):         
                if self.pool_method == "conv":
                    x4conv_kv = self.pool_layers[l](x4conv_kv)
                    x4conv_kv_all.append(x4conv_kv)
                elif self.pool_method == "conv_share":
        ...
        x4conv_kv_avg = x4conv_kv.mean(2, keepdim=True).mean(3, keepdim=True).expand(-1, -1, nH, nW)
        x4conv_kv_all.append(x4conv_kv_avg)

        x4conv_kv_all = torch.stack(x4conv_kv_all, 1).reshape(B, 2, len(x4conv_kv_all), C, nH, nW).mean(2)
        x4conv_k_all, x4conv_v_all = x4conv_kv_all[:, 0], x4conv_kv_all[:, 1]
        x_out = (self.act(x4conv_q * x4conv_k_all) * x4conv_v_all)
    ...
    def flops(self, N):
        ...
        #  self.linear
        flops += N * self.dim * (self.dim * (1+self.focal_level+1))
