...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, focal_factor, 
        ...
        self.pool_global = nn.AvgPool2d(14, padding=0)
    ...
    def forward(self, x_all, mask_all=None):
        ...
        if nH >= 28 and nW >= 28:
            x_avg = self.pool_global(x4conv).permute(0, 2, 3, 1).contiguous()
            x_avg = x_avg[:, :, None, :, None, :].expand(-1, -1, 14, -1, 14, -1).contiguous().view(x.shape)
            x_outs = [x * x_avg]
        else:
            x_avg = x.mean(1, keepdim=True).mean(2, keepdim=True).expand(-1, nH, nW, -1)
            x_outs = [x * x_avg]
        ...
        if self.pool_method != "none": 
            ...
            for l in range(self.focal_level):         
                ...
                x_avg_l = x_l.mean(1, keepdim=True).mean(2, keepdim=True).expand(-1, nH, nW, -1)
                x_outs.append(x_l * x_avg_l)
        ...
        # x_out = self.ln(x_out)
        # x_out = self.conv(x_out.permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
