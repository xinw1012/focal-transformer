...
def get_global_index_and_offset(global_size, window_size):
    # global_size: size of global input
    # window_size: window size for each global location

    global_index = torch.arange(global_size[0] * global_size[1]).view(global_size[0], global_size[1])

    global_index_all = []
    for i in range(window_size):
        for j in range(window_size):
            shift_y = window_size // 2 - i
            shift_x = window_size // 2 - j
            # if shift_x != 0 and shift_y != 0:
            #     continue
            global_index_ij = torch.roll(global_index, shifts=(shift_y, shift_x), dims=(0, 1)).view(-1)
            global_index_all.append(global_index_ij)
    global_index_all = torch.stack(global_index_all, -1)

    return global_index_all
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, window_size_glo, focal_window, 
        ...
        for k in range(self.focal_level):
            # build global index                
            local2global_window_index_k = get_global_index_and_offset((self.nWh, self.nWw), self.expand_size if k == 0 else self.focal_window)
            self.register_buffer("local2global_window_index_{}".format(k), local2global_window_index_k)

            if k == 0:
                qsize = self.window_size
                ksize = (self.window_size[0]*self.expand_size, self.window_size[1]*self.expand_size)
            else:
                qsize = self.window_size # (1, 1)
                ksize = (self.focal_window, self.focal_window)

            # build relative position bias
            relative_position_bias_table_to_windows = nn.Parameter(
                torch.zeros(
                    (qsize[0] + ksize[0] - 1) * (qsize[1] + ksize[1] - 1), 
                    self.num_heads,
                    )
            ) # 2*Wh-1 * 2*Ww-1, nH
            ...
            relative_position_index_k = get_relative_position_index(qsize, ksize)
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):        
            ...
            if self.conv_route:
                ...
                x_k_route_scores_k = x_k_route_scores_k  # B, nH, nW, nHeads

            if l_k == 0:
                x_k_route_scores_all = x_k_route_scores_k

            if l_k == 0:
                x_k_route_scores_window = window_partition_noreshape(x_k_route_scores_k, self.window_size[0])
                x_k_route_scores_window = x_k_route_scores_window.flatten(3).view(B, self.nWh*self.nWw, -1, self.num_heads)
            else:
                x_k_route_scores_window = x_k_route_scores_k.view(B, self.nWh*self.nWw, -1, self.num_heads)

            local2global_window_index_k = getattr(self, "local2global_window_index_{}".format(l_k))

            x_k_route_scores_window_local = x_k_route_scores_window[:, local2global_window_index_k.view(-1)].view(
                B, local2global_window_index_k.shape[0], -1, self.num_heads)

            topK = min(self.topK, x_k_route_scores_window_local.shape[2]); topKs.append(topK)

            topk_score_k, topk_index_k = torch.topk(x_k_route_scores_window_local, topK, dim=2) # B, topK, nHeads
            attention_bias.append(topk_score_k)
            attention_index.append(topk_index_k)

            if l_k == 0:
                k_k = k_windows.view(B, -1, self.num_heads, self.window_size[0] * self.window_size[0], C // self.num_heads)
                v_k = v_windows.view(B, -1, self.num_heads, self.window_size[0] * self.window_size[0], C // self.num_heads)
            else:
                qkv_k = self.qkv(x_k).view(B, -1, 3, self.num_heads, C // self.num_heads)
                k_k, v_k = qkv_k[:,:,1], qkv_k[:,:,2]

            k_k_local = k_k[:, local2global_window_index_k.view(-1)].view(
                B, local2global_window_index_k.shape[0], local2global_window_index_k.shape[1], self.num_heads, -1, C // self.num_heads
            ).transpose(-2, -3).view(B, local2global_window_index_k.shape[0], -1, self.num_heads, C // self.num_heads)

            v_k_local = v_k[:, local2global_window_index_k.view(-1)].view(
                B, local2global_window_index_k.shape[0], local2global_window_index_k.shape[1], self.num_heads, -1, C // self.num_heads
            ).transpose(-2, -3).view(B, local2global_window_index_k.shape[0], -1, self.num_heads, C // self.num_heads)

            k_k_selected = torch.gather(k_k_local, 2, topk_index_k.unsqueeze(-1).repeat(1, 1, 1, 1, C // self.num_heads))
            v_k_selected = torch.gather(v_k_local, 2, topk_index_k.unsqueeze(-1).repeat(1, 1, 1, 1, C // self.num_heads))

            k_k_selected = k_k_selected.view(-1, topK, self.num_heads, C // self.num_heads).transpose(1, 2)
            v_k_selected = v_k_selected.view(-1, topK, self.num_heads, C // self.num_heads).transpose(1, 2)
        ...
        if self.focal_level > 0:
            ...
            for k in range(self.focal_level):
                ...
                relative_position_index_k_selected = relative_position_index_k[:, attention_index[k].view(-1)].view(-1, self.num_heads)
                relative_position_bias_to_windows = torch.gather(self.relative_position_bias_table_to_windows[k], 0, 
                    relative_position_index_k_selected).view(
                    self.window_size[0] * self.window_size[1], B, self.nWh*self.nWw, topKs[k], self.num_heads
                )
                relative_position_bias_to_windows = relative_position_bias_to_windows.permute(1, 2, 4, 0, 3).contiguous().view(
                    -1, self.num_heads, self.window_size[0] * self.window_size[1], topKs[k])
...
def profile(model, inputs):
    ...
    with profile(activities=[
            ProfilerActivity.CPU, ProfilerActivity.CUDA], with_stack=True, profile_memory=True, record_shapes=True) as prof:
