...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, window_size_glo, focal_window, 
        ...
        for k in range(self.focal_level):
            ...
            relative_position_bias_table_to_windows = nn.Parameter(
                torch.zeros(
                    self.num_heads,
                    (2*self.window_size[0] + 2*self.expand_size - 1) * (2*self.window_size[1] + 2*self.expand_size - 1), 
                    )
            ) # 2*Wh-1 * 2*Ww-1, nH
            trunc_normal_(relative_position_bias_table_to_windows, std=.02)
            self.relative_position_bias_table_to_windows.append(relative_position_bias_table_to_windows)
            relative_position_index_k = get_relative_position_index(self.window_size, (self.window_size[0]+2*self.expand_size, self.window_size[1]+2*self.expand_size))
            self.register_buffer("relative_position_index_{}".format(k), relative_position_index_k)

            # build index map
            index_map_k = (torch.arange(self.input_resolution[0]*self.input_resolution[1])).view(self.input_resolution)[None, None, :, :].float()            
            index_map_k = F.pad(index_map_k, (self.expand_size, self.expand_size, self.expand_size, self.expand_size), value=0)
            self.register_buffer("index_map_{}".format(k), index_map_k)

            # build unfold function
            self.unfolds += [nn.Unfold(
                kernel_size=(self.window_size[0] + 2 * self.expand_size, self.window_size[1] + 2 * self.expand_size), 
                stride=self.window_size[0], padding=0)
            ]
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):
            x_k = x_all[l_k]
            x_k_route_scores = self.routers[l_k](x_k).permute(0, 3, 1, 2).contiguous()  # B, nH, nW, nHeads
            ...
            # create index for input feature map
            nH, nW = x_k_route_scores.shape[2:]

            index_map_k = getattr(self, "index_map_{}".format(l_k))
            index_map_unfolded = self.unfolds[l_k](index_map_k).view(
                1, 1, self.window_size[0]+2*self.expand_size, self.window_size[1]+2*self.expand_size, -1
            ).view((self.window_size[0]+2*self.expand_size)**2, -1)

            x_k_route_scores = F.pad(x_k_route_scores, (self.expand_size, self.expand_size, self.expand_size, self.expand_size), value=-100)
            x_k_route_scores_unfolded = self.unfolds[l_k](x_k_route_scores).view(
                B, self.num_heads, (self.window_size[0]+2*self.expand_size), (self.window_size[0]+2*self.expand_size), -1
            )

            # make the central window salient
            x_k_route_scores_unfolded[:, :, self.expand_size:(self.expand_size+self.window_size[0]), self.expand_size:(self.expand_size+self.window_size[1])] = -50
            x_k_route_scores_unfolded = x_k_route_scores_unfolded.view(B, self.num_heads, (self.window_size[0]+2*self.expand_size)**2, -1)

            topk_score_k_local, topk_index_k_local = torch.topk(x_k_route_scores_unfolded, self.topK, dim=2) # B, topK, nHeads
            attention_bias.append(topk_score_k_local)
            attention_index.append(topk_index_k_local)

            topk_index_k_local = topk_index_k_local.view(-1, topk_index_k_local.shape[-1])

            # convert local index into global index
            topk_index_k_global = torch.gather(index_map_unfolded, 0, topk_index_k_local).view(topk_score_k_local.shape)

            # since topk_index_k is local index, we need to convert it into global index based on the index_map_unfolded

            topk_index_k_offset = (topk_index_k_global.flatten(1) + ((nH*nW) * torch.arange(B).view(B, 1)).to(topk_index_k_global.device)).view(-1).long()

            k_k = k.view(-1, C)
            v_k = v.view(-1, C)
            k_k_selected = k_k[topk_index_k_offset].view(B, self.num_heads, self.topK, -1, self.num_heads, C // self.num_heads)
            v_k_selected = v_k[topk_index_k_offset].view(B, self.num_heads, self.topK, -1, self.num_heads, C // self.num_heads)

            k_k_selected = torch.diagonal(k_k_selected, dim1=1, dim2=4).permute(0, 2, 4, 1, 3).contiguous()
            v_k_selected = torch.diagonal(v_k_selected, dim1=1, dim2=4).permute(0, 2, 4, 1, 3).contiguous()
        ...
        if self.focal_level > 0:
            offset = window_area
            for k in range(self.focal_level):
                # add attention bias according to routing score
                attention_bias_k = attention_bias[k].permute(0, 3, 1, 2).contiguous().view(-1, self.num_heads, 1, self.topK)
                ...
                relative_position_index_k_selected = relative_position_index_k[:, attention_index[k].view(-1)].view((-1,)+attention_index[k].shape)
                relative_position_index_k_selected = relative_position_index_k_selected.permute(2, 1, 4, 0, 3).contiguous().flatten(1)           
                relative_position_bias_to_windows = torch.gather(self.relative_position_bias_table_to_windows[k], 1, relative_position_index_k_selected).view(
                    self.num_heads, B, self.nWh * self.nWw, -1, self.topK,
                ).permute(1, 2, 0, 3, 4).contiguous().view(B * self.nWh * self.nWw, self.num_heads, -1, self.topK)           
