...
class FocalConvolution(nn.Module):
    ...
    def forward(self, x, mask_all=None):
        ...
        x4conv_q, x4conv_kv, self.focal_weights = torch.split(x, (C, C, self.focal_level+1), 1)
        ...
        if self.pool_method != "none": 
            ...
            for l in range(self.focal_level):         
                if self.pool_method in ["conv", "pool"]:
                    ...
                    modulator = modulator + x4conv_kv*self.focal_weights[:, l:l+1]
        x_global = self.act(x4conv_kv.mean(2, keepdim=True).mean(3, keepdim=True))
        modulator = modulator + x_global*self.focal_weights[:,self.focal_level:]
        self.modulator = self.gate(modulator)
        x_out = x4conv_q*self.modulator # + modulator
...
def build_transforms4display(img_size):
    t = []
    t.append(
        transforms.Resize((img_size, img_size), interpolation=_pil_interp('bicubic'))
    )
