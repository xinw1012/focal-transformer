...
class FocalConvolution(nn.Module):
    ...
    def forward(self, x, mask_all=None):
        ...
        q, k, v = torch.split(qkv, (C, C, C), -1)
        
        k4conv = k.permute(0, 3, 1, 2).contiguous()
        v4conv = v.permute(0, 3, 1, 2).contiguous()
        ...
        if self.pool_method != "none": 
            ...
            for l in range(self.focal_level):         
                if self.pool_method in ["conv", "pool"]:
                    ...
                    k4conv = self.pool_layers[l](k4conv)
                    v4conv = self.pool_layers[l](v4conv)
                    k_all_levels.append(k4conv)
                    v_all_levels.append(v4conv)

        k_global = self.act(k4conv.mean(2, keepdim=True).mean(3, keepdim=True)).repeat(1, 1, nH, nW)
        v_global = self.act(v4conv.mean(2, keepdim=True).mean(3, keepdim=True)).repeat(1, 1, nH, nW)

        k_all_levels.append(k_global)
        v_all_levels.append(v_global)

        k_all_levels = torch.stack(k_all_levels, 1).permute(0, 1, 3, 4, 2).contiguous()
        v_all_levels = torch.stack(v_all_levels, 1).permute(0, 1, 3, 4, 2).contiguous()

        q = q.view(B, 1, nH, nW, self.num_heads, C // self.num_heads)
        q = q * self.scale

        k_all_levels = k_all_levels.view(B, self.focal_level+2, nH, nW, self.num_heads, C // self.num_heads)
        v_all_levels = v_all_levels.view(B, self.focal_level+2, nH, nW, self.num_heads, C // self.num_heads)

        attn = (q * k_all_levels).sum(-1, keepdim=True)
        attn = self.softmax(attn)
        attn = self.attn_drop(attn)
        x_out = (attn * v_all_levels).sum(1).view(B, nH, nW, C)
    ...
    def flops(self, N):
        ...
        # attn = (q @ k.transpose(-2, -1))
        flops += self.num_heads * N * (self.dim // self.num_heads) * (self.focal_level+2)
        #  x = (attn @ v)
        flops += self.num_heads * N * (self.focal_level+2) * (self.dim // self.num_heads)
