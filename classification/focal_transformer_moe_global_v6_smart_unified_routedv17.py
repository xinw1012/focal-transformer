...
class WindowAttention(nn.Module):
        ...
        if self.pool_method != "none":
            for k in range(self.focal_level):
                if self.pool_method == "conv":
                    kernel_size = min(7-2*k, 7)
                    # self.pool_layers.append(nn.Conv2d(dim, dim, kernel_size=2*(k+1)+1, stride=1, groups=dim, padding=(k+1)))
                    self.pool_layers.append(
                        nn.Sequential(
                            # nn.Conv2d(dim, dim, kernel_size=3, stride=1, groups=dim, padding=1, dilation=1),
                            nn.Conv2d(dim, dim, kernel_size=kernel_size, stride=1, groups=dim, padding=kernel_size//2),
                            # nn.ReLU(),
                            )
                        )
                    ...
                    if k == 0:
                        self.pool_layers.append(
                            nn.Sequential(
                                nn.Conv2d(dim, dim, kernel_size=2*k+3, stride=1, groups=dim, padding=k+1),
                                # nn.GELU(),
                            )
                        )                             
                    else:
    ...
    def forward(self, x_all, mask_all=None):
        ...
        q, k, v = x, x, x 
        q = q.view(B, nH, nW, self.num_heads, 1, C // self.num_heads)
        q = q * self.scale

        k_all = [k.view(B, nH, nW, self.num_heads, 1, C // self.num_heads)]
        v_all = [v.view(B, nH, nW, self.num_heads, 1, C // self.num_heads)]
        ...
        if self.pool_method != "none": 
            ...
            for l in range(self.focal_level):         
                if self.pool_method == "conv":
                    ...
                    x4qkv = x4conv.permute(0, 2, 3, 1).contiguous()
                    # qkv = self.qkv(x4qkv).reshape(B, nH, nW, 3, C).permute(3, 0, 1, 2, 4)
                    # q_k, k_k, v_k = qkv[0], qkv[1], qkv[2]  # B, nH, nW, C       
                    k_k = x4qkv.view(B, nH, nW, self.num_heads, 1, C // self.num_heads) # B, nWh, nWw, C       
                    v_k = x4qkv.view(B, nH, nW, self.num_heads, 1, C // self.num_heads) # B, nWh, nWw, C       
        ...
        k_all.append(k_all[0].mean(1, keepdim=True).mean(2, keepdim=True).expand(-1, nH, nW, -1, -1, -1))
        v_all.append(v_all[0].mean(1, keepdim=True).mean(2, keepdim=True).expand(-1, nH, nW, -1, -1, -1))
                
        k_all_routed = torch.cat(k_all, 4)
        v_all_routed = torch.cat(v_all, 4)
        
        N = k_all_routed.shape[-2]
        attn = (q @ k_all_routed.transpose(-2, -1))  # B*nW, nHead, window_size*window_size, focal_window_size*focal_window_size
        attn = self.softmax(attn)
        attn = self.attn_drop(attn)

        x = (attn @ v_all_routed).view(B, nH, nW, C)
