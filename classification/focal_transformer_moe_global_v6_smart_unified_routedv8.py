...
# add routing to select a subset of tokens
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x_all, mask_all=None):
        ...
        k_all = []; v_all = []; topKs = []; topk_rpbs = []; use_route = False
        for l_k in range(1):        
            ...
            if self.use_route and topk_rpb_k.shape[2] > self.routing_topK:
                # computer routing scores                 
                # select a subset of tokens according to routing scores
                use_route = True
                topk_rte_score_k, topk_rte_indice_k = torch.topk(topk_rpb_k.mean(1), self.routing_topK, dim=1)

                topk_indice_k_routed = torch.gather(
                    topk_closest_indice.unsqueeze(-1).repeat(1, 1, self.num_heads), 1,
                    topk_rte_indice_k
                )

                topk_rpb_k = torch.gather(
                    topk_rpb_k, 2, topk_rte_indice_k.unsqueeze(1).repeat(1, topk_rpb_k.shape[1], 1, 1)
                )

                topk_rpb_k = topk_rpb_k.unsqueeze(0).repeat(B, 1, 1, 1, 1)
                topk_rpb_k = topk_rpb_k.view(-1, *topk_rpb_k.shape[2:])

                topk_indice_k_routed = topk_indice_k_routed.unsqueeze(0).repeat(B, 1, 1, 1)
                ...
                k_k_selected = torch.gather(k_k, 1, topk_indice_k_routed.view(B, -1, self.num_heads).unsqueeze(-1).repeat(1, 1, 1, C // self.num_heads)).view(
                    *topk_indice_k_routed.shape, C // self.num_heads).transpose(2, 3)
                ...
                k_all.append(k_k_selected.view(-1, *k_k_selected.shape[2:]))
                v_all.append(v_k_selected.view(-1, *k_k_selected.shape[2:]))
        ...
        k_all_routed = k_all[0]
        v_all_routed = v_all[0]
        ...
        if self.pool_method != "none":
            for k in range(self.focal_level-1):
                ...
                if self.pool_method == "conv":
                    ...
                    self.pool_layers.append(nn.Conv2d(1, 1, kernel_size=window_pool_size+4, stride=window_pool_size, groups=1, padding=2))
        ...
        if self.focal_level > 1 and self.pool_method != "none": 
            ...
            for k in range(self.focal_level-1):         
                ...
                if self.pool_method == "conv":
                    ...
                    x_windows_pooled = self.pool_layers[k](x_p)
                    x_windows_pooled = x_windows_pooled.view(B, C, *x_windows_pooled.shape[2:]).permute(0, 2, 3, 1).contiguous() # B, nWh, nWw, C           

                x_windows_all[0] = x_windows_all[0] + \
                    x_windows_pooled.unsqueeze(3).unsqueeze(2).repeat(
                        1, 1, x_windows_all[0].shape[1] // x_windows_pooled.shape[1], 1,  x_windows_all[0].shape[2] // x_windows_pooled.shape[2], 1).view(x_windows_all[0].shape)
