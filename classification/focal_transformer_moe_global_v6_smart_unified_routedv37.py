...
class WindowAttention(nn.Module):
    ...
    def forward(self, x_all, mask_all=None):
        ...
        x_all.append(x.mean(1, keepdim=True).mean(2, keepdim=True).expand(-1, nH, nW, -1))
        x_out = torch.stack(x_all, -2)

        qkv = self.qkv(x_out).reshape(B, nH, nW, len(x_all), 3, C)
        q, k, v = qkv[:,:,:,:,0], qkv[:,:,:,:,1], qkv[:,:,:,:,2]

        q = q.reshape(B, nH, nW, -1, self.num_heads, C // self.num_heads).transpose(3, 4)
        k = k.reshape(B, nH, nW, -1, self.num_heads, C // self.num_heads).transpose(3, 4)
        v = v.reshape(B, nH, nW, -1, self.num_heads, C // self.num_heads).transpose(3, 4)

        q = q * self.scale
        attn = q @ k.transpose(-2, -1)
        attn = self.softmax(attn)
        attn = self.attn_drop(attn)
        x_out = (attn @ v)[:,:,:,:, 0].reshape(B, nH, nW, C)
