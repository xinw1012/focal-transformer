...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, 
        ...
        self.mute_fine_grain = mute_fine_grain

        if not self.mute_fine_grain:
            for k in range(1):
    ...
    def forward(self, x_all, mask_all=None):
        ...
        if self.focal_level > 1 and topk_rpbs_routed is not None:
            attn[:,:,:,:-(self.focal_level-1)] = attn[:,:,:,:-(self.focal_level-1)] + topk_rpbs_routed.permute(0, 3, 1, 2)
        elif topk_rpbs_routed is not None:
...
if __name__ == '__main__':
    ...
    model = FocalTransformer(
        img_size=img_size, 
        embed_dim=64, 
        depths=[2,2,16,2], drop_path_rate=0.2, 
        focal_levels=[4,3,3,2], 
