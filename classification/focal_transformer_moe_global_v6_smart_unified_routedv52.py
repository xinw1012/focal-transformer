...
class FocalConvolution(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, focal_factor, 
        ...
        self.gate = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0, groups=self.num_heads, bias=qkv_bias), 
            # nn.Sigmoid(),
        )
        ...
        self.focal_weights = nn.Conv2d(dim, self.focal_level+1, kernel_size=1, stride=1)
        self.kernel_sizes = []
        ...
        self.global_pool_layer = nn.Sequential(
            nn.Linear(self.input_resolution[0]*self.input_resolution[1], 1), 
            nn.GELU(),
        )
    ...
    def forward(self, x, mask_all=None):
        ...
        focal_weights = self.focal_weights(x4conv_kv)
        if self.pool_method != "none": 
        ...
        x_global = self.global_pool_layer(x4conv_kv.flatten(2)).unsqueeze(-1)
        modulator = modulator + x_global*focal_weights[:, -1:]
