...
# changelog compared with v15
# use bn instead of ln
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, 
        ...
        self.linear = nn.Linear(dim*(1+self.focal_level+1), dim, bias=qkv_bias)
        # self.conv = nn.Conv2d(dim*(1+self.focal_level), dim, kernel_size=3, stride=1,padding=1)
        ...
        if self.pool_method != "none":
            for k in range(self.focal_level):
                # kernel_size = min(2*k+3, 5)
                kernel_size = max(7-2*k, 3)
                if self.pool_method == "conv":
                    ...
                    self.pool_layers.append(
                        nn.Sequential(
                            ...
                            groups=dim, padding=kernel_size//2, padding_mode="zeros"),
                            # nn.BatchNorm2d(dim),
                            # nn.GELU(),
                            )
                        )
        ...
        if self.pool_method != "none": 
            ...
            for l in range(self.focal_level):         
                if self.pool_method == "conv":
                    ...
                    # x4conv_out = self.pool_layers[l](x4conv)
                    # x_l = x4conv_out.permute(0, 2, 3, 1).contiguous() # B, nWh, nWw, C                           
                elif self.pool_method == "conv_share":
                    k = self.pool_layers[l](k)                    
                    x_p_k = x_p.view(B, C, *x_p.shape[2:]).permute(0, 2, 3, 1).contiguous().view(-1, self.num_heads, 1, C // self.num_heads) # B, nWh, nWw, C           
                x_outs.append(x_l)

        x_outs.append(x_outs[-1].mean(1, keepdim=True).mean(2, keepdim=True).expand(-1, nH, nW, -1))
        x_outs = torch.cat(x_outs, -1)
        
        x_out = self.linear(x_outs)
        # x_out = self.conv(x_outs.permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
    ...
    def flops(self, N):
        ...
        # focal convolution
        for k in range(self.focal_level):
            flops += N * (2*k+3) * self.dim

        #  self.linear
        flops += N * self.dim * (self.dim * (1 + self.focal_level))
...
class FocalTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.drop_path(self.mlp(self.norm2(x.transpose(1, 2)).transpose(1, 2)) if (not self.use_layerscale) \
            else (self.gamma_2 * self.mlp(self.norm2(x.transpose(1, 2)).transpose(1, 2))))
    ...
    def __init__(self, img_size=(224, 224), patch_size=4, in_chans=3, embed_dim=96, use_conv_embed=False, norm_layer=None, use_pre_norm=False, is_stem=False):
        ...
        if use_conv_embed:
            ...
            if is_stem:
                ...
                #     nn.Conv2d(in_chans, embed_dim // 3, kernel_size=3, stride=2, padding=1), 
                #     nn.GELU(),                    
                #     nn.Conv2d(embed_dim // 3, embed_dim, kernel_size=3, stride=2, padding=1), 
                # )
...
class FocalTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            x = layer(x)
        x = self.norm(x.transpose(1, 2)).transpose(1, 2)  # B L C
...
if __name__ == '__main__':
    ...
    model = FocalTransformer(
        img_size=img_size, 
        embed_dim=64, 
        depths=[1,2,21,2], drop_path_rate=0.5, 
        focal_levels=[1,1,1,1], 
