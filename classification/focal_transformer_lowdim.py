...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, expand_size, window_size, focal_window, focal_level, num_heads, 
        ...
        self.qkv = nn.Linear(dim, 2*self.num_heads*16 + dim, bias=qkv_bias)
    ...
    def forward(self, x_all, mask_all=None):
        ...
        B, nH, nW, C = x.shape
        qkv = self.qkv(x).reshape(B, nH, nW, 2*self.num_heads*16 + C) # .permute(3, 0, 1, 2, 4).contiguous()
        q, k, v = qkv[:,:,:,:self.num_heads*16], qkv[:,:,:,self.num_heads*16:self.num_heads*32], qkv[:,:,:,self.num_heads*32:]  # B, nH, nW, C

        # partition q map
        (q_windows, k_windows) = map(
            lambda t: window_partition(t, self.window_size[0]).view(
            -1, self.window_size[0] * self.window_size[0], self.num_heads, 16
            ).transpose(1, 2), 
            (q, k)
        )
        ...
        if self.expand_size > 0 and self.focal_level > 0:
            ...
            (k_tl_windows, k_tr_windows, k_bl_windows, k_br_windows) = map(
                lambda t: window_partition(t, self.window_size[0]).view(-1, self.window_size[0] * self.window_size[0], self.num_heads, 16), 
                (k_tl, k_tr, k_bl, k_br)
            )            
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(self.focal_level-1):
                ...
                qkv_pooled = self.qkv(x_window_pooled).reshape(B, nWh, nWw, 2*self.num_heads*16 + C).permute(0, 3, 1, 2).contiguous()
                k_pooled_k, v_pooled_k = qkv_pooled[:, self.num_heads*16:self.num_heads*32], qkv_pooled[:,self.num_heads*32:]  # B, C, nWh, nWw

                k_pooled_k = self.unfolds[k](k_pooled_k).view(
                    B, self.num_heads*16, self.unfolds[k].kernel_size[0], self.unfolds[k].kernel_size[1], -1).permute(0, 4, 2, 3, 1).contiguous().\
                    view(-1, self.unfolds[k].kernel_size[0]*self.unfolds[k].kernel_size[1], self.num_heads, 16).transpose(1, 2)
                # (B x (nH*nW)) x nHeads x (unfold_wsize x unfold_wsize) x head_dim

                v_pooled_k = self.unfolds[k](v_pooled_k).view(
