...
def get_topk_closest_indice(input_resolution, window_size_q, window_size_k, topk=1, dist_norm="l2"):
    # get pair-wise relative position index for each token inside the window
    q_windows = (
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, 
                    ...
                    use_route=False, topK=64, routing_topK=64, dist_norm="l2"):
        ...
        for k in range(self.focal_level):
            ...
            if k > 0:
                window_pooler = nn.Conv2d(dim, dim, kernel_size=self.focal_window, stride=self.focal_window, groups=dim)
                self.window_poolers.append(window_pooler)
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):        
            ...
            if l_k == 0:
                ...
                k_k = self.window_poolers[l_k-1](k.permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous().view(B, -1, self.num_heads, C // self.num_heads)
                v_k = self.window_poolers[l_k-1](v.permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous().view(B, -1, self.num_heads, C // self.num_heads)
                # qkv_k = self.qkv(x_k).view(B, -1, 3, self.num_heads, C // self.num_heads)
                # k_k, v_k = qkv_k[:,:,1], qkv_k[:,:,2]

            if self.use_route:
                # computer routing scores                
                rte_score_k = self.routers[l_k](v_k.flatten(-2)).view(B, -1, self.num_heads)
                v_k = v_k * torch.sigmoid(rte_score_k).unsqueeze(-1)
        ...
        # k_merged = k_all[0]; v_merged = v_all[0]; topk_rpbs_merged = topk_rpbs[0]
        # for l in range(self.focal_level-1):
        #     k_merged[:,:,:k_all[l+1].shape[2]] = k_merged[:,:,:k_all[l+1].shape[2]] + k_all[l+1]
        #     v_merged[:,:,:v_all[l+1].shape[2]] = v_merged[:,:,:v_all[l+1].shape[2]] + v_all[l+1]
        #     topk_rpbs_merged[:,:,:topk_rpbs[l+1].shape[2]] = topk_rpbs_merged[:,:,:topk_rpbs[l+1].shape[2]] + topk_rpbs[l+1]

        (k_merged, v_merged, topk_rpbs_merged) = map(
        ...
        k_all_routed = k_merged
        v_all_routed = v_merged
        topk_rpbs_routed = topk_rpbs_merged
...
class FocalTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, expand_size=0, shift_size=0,
                 ...
                 focal_level=1, focal_window=1, topK=64, routing_topK=64, dist_norm="l2", use_layerscale=False, layerscale_value=1e-4):
        super().__init__()
        self.dim = dim
        ...
        self.attn = WindowAttention(
            ...
            pool_method=pool_method, use_route=use_route, topK=topK, routing_topK=routing_topK, dist_norm=dist_norm)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, expand_size, expand_layer,
                 ...
                 focal_level=1, focal_window=1, topK=64, routing_topK=64, use_conv_embed=False, use_shift=False, use_pre_norm=False, 
