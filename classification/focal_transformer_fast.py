...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, window_size_glo, focal_window, 
        ...
        self.unfold = nn.Unfold(kernel_size=(3,3), stride=1, padding=1)
    ...
    def forward(self, x_all, mask_all=None):
        ...
        if self.expand_size > 0 and self.focal_level > 0:
            tic = time.time()
            k_windows4unfold = k_windows.view(B, nH // self.window_size[0], nW // self.window_size[1], self.num_heads, 
                    self.window_size[0] * self.window_size[0], C // self.num_heads)
            v_windows4unfold = v_windows.view(B, nH // self.window_size[0], nW // self.window_size[1], self.num_heads, 
                    self.window_size[0] * self.window_size[0], C // self.num_heads)

            (k_tl, v_tl) = map(
                lambda t: torch.roll(t, shifts=(-1, -1), dims=(1, 2)), (k_windows4unfold, v_windows4unfold)
            )
            (k_tr, v_tr) = map(
                lambda t: torch.roll(t, shifts=(-1, 1), dims=(1, 2)), (k_windows4unfold, v_windows4unfold)
            )
            ...
            (k_t, v_t) = map(
                lambda t: torch.roll(t, shifts=(-1), dims=(1)), (k_windows4unfold, v_windows4unfold)
            )
            (k_r, v_r) = map(
                lambda t: torch.roll(t, shifts=(-1), dims=(2)), (k_windows4unfold, v_windows4unfold)
            )
            (k_b, v_b) = map(
                lambda t: torch.roll(t, shifts=(1), dims=(1)), (k_windows4unfold, v_windows4unfold)
            )
            ...
            k_rolled = torch.cat((k_tl, k_t, k_tr, k_r, k_br, k_b, k_bl, k_l), 4).view(-1, self.num_heads, 8*self.window_size[0]*self.window_size[0], C // self.num_heads)
            v_rolled = torch.cat((v_tl, v_t, v_tr, v_r, v_br, v_b, v_bl, v_l), 4).view(-1, self.num_heads, 8*self.window_size[0]*self.window_size[0], C // self.num_heads)
...
if __name__ == '__main__':
    ...
    model = FocalTransformer(embed_dim=96, depths=[2,2,6,2], drop_path_rate=0.2, 
        focal_levels=[2,2,2,2], expand_sizes=[3,3,3,3], expand_layer="all", use_conv_embed=False, use_shift=False, 
    ).cuda()
    model.eval()
