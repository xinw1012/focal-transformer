...
# changelog compared with v3
# make relative position range bias learnable, instead of fixed, share router across heads
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):        
            ...
            if self.conv_route:
                ...
                x_k_route_scores_k = self.routers[l_k](x_k4conv).view(B, 1, -1).transpose(1, 2).repeat(1, 1, self.num_heads)
            else:
                x_k_route_scores_k = self.routers[l_k](x_k)
                x_k_route_scores_k = x_k_route_scores_k.view(B, -1, 1).repeat(1, 1, self.num_heads) # B, nH, nW, nHeads
