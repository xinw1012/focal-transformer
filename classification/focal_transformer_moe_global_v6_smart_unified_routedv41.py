...
class WindowAttention(nn.Module):
    ...
    def forward(self, x_all, mask_all=None):
        ...
        x4conv_k_all = self.v(x4conv_kv)
        # x4conv_v_all = x4conv_kv
        if self.pool_method != "none": 
            # if we add coarser granularity and the pool method is not none
            for l in range(self.focal_level):         
                if self.pool_method == "conv":
                    x4conv_kv = self.pool_layers[l](x4conv_kv)
                    # x4conv_v_all = x4conv_v_all + x4conv_kv * self.focal_weights[l]
                    x4conv_k_all = x4conv_k_all + self.v(x4conv_kv) * self.focal_weights[l]

                    # x4conv_q = self.pool_layers[l](x4conv_q)
                    # x4conv_q_all = x4conv_q_all + x4conv_q
        ...
        # x4conv_v_all = x4conv_v_all + x4conv_kv.mean(2, keepdim=True).mean(3, keepdim=True) * self.focal_weights[-1]
        x4conv_k_all = x4conv_k_all + self.v(x4conv_kv.mean(2, keepdim=True).mean(3, keepdim=True)) * self.focal_weights[-1]

        x_out = x4conv_q_all * x4conv_k_all
...
if __name__ == '__main__':
    ...
    ckpt_path = "/datadrive/azureblobs/model_storage2/projects/focalv3/amlt-results/7357714230.24229-9e5bc77b-f432-4ac8-9cf7-1e23c341685c/ckpt_epoch_288.pth"
