...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, window_size_glo, focal_window, 
        ...
        self.window_size_of_key = self.window_size[0] * self.window_size[1] if (self.expand_size == 0 or self.focal_level == 0) else \
            ...
            for k in range(focal_level-1):
                ...
                relative_position_bias_table_xwin = nn.Parameter(
                    torch.zeros(
                        ...
                        self.focal_window*self.focal_window, 
                        )
                ) # 2*Wh-1 * 2*Ww-1, nH
                ...
                # self.qkv_global.append(nn.Linear(dim, dim * 3, bias=qkv_bias))
    ...
    def forward(self, x_all, mask_all=None):
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            # if self.focal_window == 7:
            #     k_all = torch.cat((k_rolled, k_rolled.new(k_rolled.shape[0], k_rolled.shape[1], 49, k_rolled.shape[3])), 2)
            #     v_all = torch.cat((v_rolled, v_rolled.new(v_rolled.shape[0], v_rolled.shape[1], 49, v_rolled.shape[3])), 2)
            # elif self.focal_window == 5:
            #     k_all = torch.cat((k_rolled, k_rolled.new(k_rolled.shape[0], k_rolled.shape[1], 25, k_rolled.shape[3])), 2)
            #     v_all = torch.cat((v_rolled, v_rolled.new(v_rolled.shape[0], v_rolled.shape[1], 25, v_rolled.shape[3])), 2)
            # elif self.focal_window == 3:
            #     k_all = torch.cat((k_rolled, k_rolled.new(k_rolled.shape[0], k_rolled.shape[1], 9, k_rolled.shape[3])), 2)
            #     v_all = torch.cat((v_rolled, v_rolled.new(v_rolled.shape[0], v_rolled.shape[1], 9, v_rolled.shape[3])), 2)
            # elif self.focal_window == 1:
            #     k_all = torch.cat((k_rolled, k_rolled.new(k_rolled.shape[0], k_rolled.shape[1], 1, k_rolled.shape[3])), 2)
            #     v_all = torch.cat((v_rolled, v_rolled.new(v_rolled.shape[0], v_rolled.shape[1], 1, v_rolled.shape[3])), 2)
        else:
            k_all = k_rolled
            v_all = v_rolled
        ...
        # if self.focal_window == 7:
        #     k_all = torch.cat((k_all, k_all.new(k_all.shape[0], k_all.shape[1], 132, k_all.shape[3])), 2)
        #     v_all = torch.cat((v_all, v_all.new(v_all.shape[0], v_all.shape[1], 132, v_all.shape[3])), 2)
        # elif self.focal_window == 5:
        #     k_all = torch.cat((k_all, k_all.new(k_all.shape[0], k_all.shape[1], 132, k_all.shape[3])), 2)
        #     v_all = torch.cat((v_all, v_all.new(v_all.shape[0], v_all.shape[1], 132, v_all.shape[3])), 2)
        # elif self.focal_window == 3:
        ...
        if self.pool_method != "none":
            ...
            for k in range(self.focal_level-1):
                ...
                attn[:, :, :window_area, offset:(offset + self.focal_window**2)] = \
                    attn[:, :, :window_area, offset:(offset + self.focal_window**2)] + relative_position_bias_xwin.unsqueeze(0)
    ...
    def flops(self):
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            for k in range(self.focal_level-1):
                window_size_glo = math.floor(self.window_size_glo / (2 ** k))
                # (sub)-window pooling
                flops += nW * self.dim * window_size_glo * window_size_glo         
                # kv for global
...
if __name__ == '__main__':
    ...
    model = FocalTransformer(img_size=img_size, embed_dim=96, depths=[2,2,6,2], drop_path_rate=0.2, 
        focal_levels=[2,2,2,2], expand_sizes=[3,3,3,0], expand_layer="all", 
        focal_windows=[7, 5, 3, 1], 
