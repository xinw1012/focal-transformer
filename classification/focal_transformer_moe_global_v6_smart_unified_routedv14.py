...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, head_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
        ...
        self.k = nn.Linear(head_features, in_features)
        self.v = nn.Linear(head_features, out_features)
    ...
    def forward(self, x, head_embedding):
        x_q = self.q(x)
        head_k = self.k(head_embedding.T)
        head_v = self.v(head_embedding.T)
        attn = x_q @ head_k.unsqueeze(0).transpose(1, 2)
        attn = self.softmax(attn)
        attn = self.attn_drop(attn)
        x_out = attn @ head_v.unsqueeze(0)
        return x_out
...
class MlpSimple(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, head_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.k = nn.Parameter(torch.zeros(head_features, out_features))
        self.v = nn.Parameter(torch.zeros(head_features, out_features))
    ...
    def forward(self, x, head_embedding):
        ...
        head_k = head_embedding.t() @ self.k
        head_v = head_embedding.t() @ self.v
        attn = x @ head_k.unsqueeze(0).transpose(1, 2)
        # attn = self.softmax(attn)
        attn = self.act(attn)
        attn = self.drop(attn)
        x_out = attn @ head_v.unsqueeze(0)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, 
        ...
        self.coarse_grain_bias = nn.Parameter(torch.zeros(self.focal_level))
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(0, self.focal_level):
            x_lk = x_all[l_k+1]
            qkv_lk = self.qkv(x_lk).reshape(B, *x_lk.shape[1:3], 3, C).permute(3, 0, 1, 2, 4)
            k_lk, v_lk = qkv_lk[1], qkv_lk[2]  # B, nH, nW, C
        ...
        attn[:,:,:,:attn.shape[2]] = attn[:,:,:,:attn.shape[2]] + relative_position_bias.unsqueeze(0)
        attn[:,:,:,attn.shape[2]:] = attn[:,:,:,attn.shape[2]:] + self.coarse_grain_bias[None, None, None, :]
...
class FocalTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, expand_size=0, shift_size=0,
                 ...
                 focal_level=1, focal_window=1, focal_kernel=3, topK=64, routing_topK=64, shape="circle", head_dim=None, 
                ...
                if self.pool_method == "conv":
                    ...
                    if k == 0:
                        self.pool_layers.append(
                            nn.Conv2d(dim, dim, kernel_size=self.window_size, stride=self.window_size, groups=dim, padding=0)
                        )
                    else:
                        self.pool_layers.append(
                            nn.Conv2d(dim, dim, kernel_size=focal_kernel, stride=1, groups=dim, padding=focal_kernel//2)
                        )
                    ...
                    if k == 0:
                        self.pool_layers.append(
                            nn.Sequential(
                                nn.Conv2d(1, 1, kernel_size=window_pool_size+6, stride=window_pool_size, groups=1, padding=3),
                                # nn.GELU(),
                            )
                        )                             
                    else:
        ...
        self.mlp = MlpSimple(in_features=dim, hidden_features=mlp_hidden_dim, head_features=self.head_dim, act_layer=act_layer, drop=drop)
    ...
    def forward(self, x, head_embedding):
        ...
        if self.pool_method == "conv_share":
        ...
        if self.pool_method != "none": 
            ...
            for k in range(self.focal_level):         
                if self.pool_method == "conv":
                    ...
                    x_p_k = x_p.permute(0, 2, 3, 1).contiguous() # B, nWh, nWw, C           
                elif self.pool_method == "conv_share":
                    x_p = self.pool_layers[k](x_p)                    
                    x_p_k = x_p.view(B, C, *x_p.shape[2:]).permute(0, 2, 3, 1).contiguous() # B, nWh, nWw, C           
                x_all.append(x_p_k)
        ...
        x = x + self.drop_path(self.mlp(self.norm2(x), head_embedding) if (not self.use_layerscale) else (self.gamma_2 * self.mlp(self.norm2(x))))
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, expand_size, expand_layer,
                 ...
                 downsample=None, use_checkpoint=False, use_layerscale=False, layerscale_value=1e-4, head_dim=None):
    ...
    def forward(self, x, head_embedding):
        for blk in self.blocks:
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, head_embedding)
            else:
                x = blk(x, head_embedding)
...
class FocalTransformer(nn.Module):
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                            ...
                            #    downsample=PatchEmbed if (i_layer < self.num_layers - 1) else None,
                               ...
                               use_route=use_route, 
                               layerscale_value=layerscale_value, 
                               head_dim=self.num_features)
            self.layers.append(layer)

        self.norm = norm_layer(self.num_features)
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.head_embedding = nn.Parameter(torch.zeros(self.num_features, num_classes))
        self.head_bias = nn.Parameter(torch.zeros(1, num_classes))
    ...
    def forward_features(self, x, head_embedding):
    ...
    def forward(self, x):
        x = self.forward_features(x, self.head_embedding)
        x = x @ self.head_embedding + self.head_bias        
        return x
...
if __name__ == '__main__':
    ...
    model = FocalTransformer(
        ...
        focal_levels=[3,3,2,1], 
        focal_kernels=[5,3,3,3], 
        focal_windows=[7,7,7,7], 
        dilation=1, 
        # num_heads=[2,4,8,16],
        # num_heads=[4,8,16,32],
