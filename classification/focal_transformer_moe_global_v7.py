...
# changelog compared with v6
# fine-grained relative position bias for focal level 1, but seems not work well
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):        
            ...
            if self.conv_route:
                ...
                x_k_route_scores_k = x_k_route_scores_k.view(B, -1, 1) # B, nH, nW, nHeads

            x_k_route_scores_all.append(x_k_route_scores_k.view(B, x_k.shape[1], x_k.shape[2], 1))
            ...
            topk_score_k, topk_index_k = torch.topk(x_k_route_scores_positioned, topK, dim=2) # B, topK, 1

            # topk_score_k = topk_score_k.repeat(1, 1, 1, self.num_heads)
            # topk_index_k = topk_index_k.repeat(1, 1, 1, self.num_heads)
            ...
            topk_index_k_duplicated = topk_index_k.view(B, -1, 1).unsqueeze(-1).repeat(1, 1, self.num_heads, C // self.num_heads)
            k_k_selected = torch.gather(k_k, 1, topk_index_k_duplicated)
            v_k_selected = torch.gather(v_k, 1, topk_index_k_duplicated)

            k_k_selected = k_k_selected.view((-1,) + topk_index_k.shape[2:4] + (self.num_heads, C // self.num_heads)).transpose(1, 2)
            v_k_selected = v_k_selected.view((-1,) + topk_index_k.shape[2:4] + (self.num_heads, C // self.num_heads)).transpose(1, 2)
        ...
        if x_k_route_scores_all is not None:
            route_scores = window_partition(x_k_route_scores_all[0], self.window_size[0]).permute(0, 3, 1, 2).contiguous().flatten(2).unsqueeze(2)
