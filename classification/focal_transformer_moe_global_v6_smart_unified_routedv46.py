        ...
        if self.pool_method != "none": 
            ...
            for l in range(self.focal_level):         
                if self.pool_method in ["conv", "pool"]:
                    ...
                    modulator = modulator + x4conv_q*x4conv_kv*self.focal_weights[l]
        x_global = self.act(x4conv_kv.mean(2, keepdim=True).mean(3, keepdim=True))
        modulator = modulator + x4conv_q*x_global*self.focal_weights[-1]
        x_out = x4conv_q*self.gate(modulator) + x4conv_kv
...
class FocalTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        if isinstance(embed_dim, list) and len(embed_dim) == 1:
...
if __name__ == '__main__':
    ...
    ckpt_path = "/datadrive/azureblobs/model_storage3/imagenet22k/focalv3-large-lr0.0005-wd0.05-bs4096-clip5.0-g32/large.yaml_conf~/run_1/best_model/model/default/model_state_dict.pt"
