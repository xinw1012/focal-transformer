        ...
        if pool_method != "none" and focal_level > 1:
            ...
            for k in range(focal_level-1):
                ...
                if k > 0:
                    ...
                    for i in range(2**k):
                        for j in range(2**k):
                            mask = torch.ones(self.focal_window, self.focal_window)
                            mask_padded = F.pad(mask, (j, 2**k-j-1, i, 2**k-i-1), value=-100.0)
                            mask_padded_all.append(mask_padded)                        
                    mask_padded_all = torch.stack(mask_padded_all, 2)
                    mask_padded_index = mask_padded_all.max(2)[1]

                    selected_index = []
                    for i in range(2**k):
                        for j in range(2**k):
                            mask_padded_index_trim = mask_padded_index[i:(i+self.focal_window), j:(j+self.focal_window)].contiguous()
                            selected_index.append((i*(2**k) + j)*(self.focal_window**2) + (mask_padded_index_trim.view(-1) == (i*(2**k) + j)).nonzero())
                    selected_index = torch.cat(selected_index, 0)
                    self.register_buffer("valid_ind_unfold_{}".format(k), selected_index.flatten(0))
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(self.focal_level-1):
                ...
                unfolded_mask = self.unfolds[k](mask.unsqueeze(0).unsqueeze(1)).view(
                    ...
                    view(nWh // stride, stride, nWw // stride, stride, self.focal_window**2, 1)
                unfolded_mask = unfolded_mask.permute(0, 2, 1, 3, 4, 5).contiguous().view(-1, stride*stride*(self.focal_window**2), 1)
                ...
                (k_pooled_k, v_pooled_k) = map(
                    ...
                    view(B, nWh // stride, stride, nWw // stride, stride, self.focal_window**2, self.num_heads, C // self.num_heads), 
                    (k_pooled_k, v_pooled_k)  # (B x (nH*nW)) x nHeads x (unfold_wsize x unfold_wsize) x head_dim
                )
                
                (k_pooled_k, v_pooled_k) = map(
                    lambda t: t.permute(0, 1, 3, 2, 4, 5, 6, 7).contiguous().view(-1, stride*stride*(self.focal_window**2), self.num_heads, C // self.num_heads).transpose(1, 2), 
