...
# constrain constant number of neighbors for accelerating sorting 
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x_all, mask_all=None):
        ...
        # we first compute the routing scores
        routing_scores = self.routers[0](x) # B, nH, nW, 1

        # window partition
        x_window = window_partition(x, self.window_size[0]).view(B*self.nWh*self.nWw, -1, C)        
        routing_scores_window = window_partition(routing_scores, self.window_size[0]).flatten(1)

        # select top k, e.g., 25
        topK_in_window = 5*5
        topk_score, topk_index = torch.topk(routing_scores_window, topK_in_window, dim=1) # B*nWh*nWw, topK, 1

        topk_score = topk_score.view(B, self.nWh*self.nWw, topK_in_window)
        # gather top k features
        x_window_selected = torch.gather(x_window, 1, topk_index.unsqueeze(-1).repeat(1, 1, C))

        q_windows = self.q(x_window).view(B*self.nWh*self.nWw, self.window_size[0]*self.window_size[1], self.num_heads, C // self.num_heads).transpose(1, 2)
        # q_windows = self.q(x_window_selected).view(B*self.nWh*self.nWw, x_window_selected.shape[1], self.num_heads, C // self.num_heads).transpose(1, 2)

        k_windows_selected = self.k(x_window_selected).view(B*self.nWh*self.nWw, topK_in_window, self.num_heads, C // self.num_heads).transpose(1, 2)

        v_windows = self.v(x_window).view(B*self.nWh*self.nWw, self.window_size[0]*self.window_size[1], C)
        v_windows_selected = torch.gather(v_windows, 1, topk_index.unsqueeze(-1).repeat(1, 1, C))
        v_windows_selected = v_windows_selected.view(B*self.nWh*self.nWw, topK_in_window, self.num_heads, C // self.num_heads).transpose(1, 2)
        ...
        for l_k in range(self.focal_level):        
            ...
            k_windows_selected = torch.gather(k_windows_t, 1, topk_index_k.flatten(1)[:, :, None, None].repeat(1, 1, self.num_heads, C // self.num_heads))
            v_windows_selected = torch.gather(v_windows_t, 1, topk_index_k.flatten(1)[:, :, None, None].repeat(1, 1, self.num_heads, C // self.num_heads))

            k_windows_selected = k_windows_selected.view((-1, topK) + (self.num_heads, C // self.num_heads)).transpose(1, 2)
            v_windows_selected = v_windows_selected.view((-1, topK) + (self.num_heads, C // self.num_heads)).transpose(1, 2)

            k_all = torch.cat((k_all, k_windows_selected), 2)
            v_all = torch.cat((v_all, v_windows_selected), 2)
        
        self.route_scores = routing_scores
        ...
        if window_area < self.window_size[0]*self.window_size[1]:
            relative_position_bias = torch.gather(relative_position_bias, 2, topk_index[:, None, :, None].repeat(1, self.num_heads, 1, relative_position_bias.shape[-1]))
            relative_position_bias = torch.gather(relative_position_bias, 3, topk_index[:, None, None, :].repeat(1, self.num_heads, relative_position_bias.shape[-2], 1))
        else:
            relative_position_bias = torch.gather(relative_position_bias, 3, topk_index[:, None, None, :].repeat(1, self.num_heads, relative_position_bias.shape[-2], 1))
        ...
        if window_area < self.window_size[0]*self.window_size[1]:
            x = torch.scatter(v_windows, 1, topk_index.unsqueeze(-1).repeat(1, 1, C), x)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
