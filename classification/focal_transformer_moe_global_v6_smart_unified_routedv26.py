...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, focal_factor, 
        ...
        self.conv = nn.Conv2d(dim*(1+self.focal_level+1), dim, kernel_size=1, stride=1, padding=0, groups=3, bias=True)
        # self.linear = nn.Linear(dim, dim, bias=qkv_bias)
    ...
    def forward(self, x_all, mask_all=None):
        ...
        x_outs.append(x_outs[0].mean(2, keepdim=True).mean(3, keepdim=True).expand(-1, -1, nH, nW))
        x_out = torch.stack(x_outs, 2).view(B, -1, nH, nW)

        x_out = self.conv(x_out).permute(0, 2, 3, 1).contiguous()
        # x_out = self.ln(x_out)
