...
# changelog compared with last version
# construct fine-grained rleative position bias, previously, we define relative position bias for each window
# seems like it can not model the fine-grained tokens especially for later stages.
...
def get_relative_position_distance(q_windows, k_windows, valid_range=1):
    # get pair-wise relative position index for each token inside the window
    coords_h_q = torch.arange(q_windows[0])
    coords_w_q = torch.arange(q_windows[1])
    coords_q = torch.stack(torch.meshgrid([coords_h_q, coords_w_q]))  # 2, Wh_q, Ww_q
    ...
    threshold = (valid_range + valid_range)
    relative_position_dists[relative_position_dists <= threshold] = 0.0
    relative_position_dists[relative_position_dists > threshold] = -100000.0
    # exclude diagonal elements
    # relative_position_dists[relative_position_dists == 0] = -100000.0

    return relative_position_dists
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, window_size_glo, focal_window, 
        ...
        # self.relative_position_bias_table_in_windows = nn.ParameterList()

        for k in range(self.focal_level):
            # build relative position range            
            relative_position_range_k_init = get_relative_position_distance(
                (self.nWh, self.nWw), (self.nWh, self.nWw), 
                valid_range=(1 if k==0 else 100))
            ...
            if k == 0:
                partition_window = self.input_resolution
            else:
                partition_window = (self.nWh, self.nWw)

            relative_position_bias_table_to_windows = nn.Parameter(
                torch.zeros(
                    (self.input_resolution[0] + partition_window[0] - 1) * (self.input_resolution[1] + partition_window[1] - 1), 
                    self.num_heads,
                    )
            ) # 2*Wh-1 * 2*Ww-1, nH
            ...
            relative_position_index_k = get_relative_position_index(self.input_resolution, partition_window)
            self.register_buffer("relative_position_index_{}".format(k), relative_position_index_k)

            # relative_position_bias_table_in_windows = nn.Parameter(
            #     torch.zeros(
            #         self.window_size[0]*self.window_size[1],
            #         self.num_heads,
            #         )
            # )
            # trunc_normal_(relative_position_bias_table_in_windows, std=.02)
            # self.relative_position_bias_table_in_windows.append(relative_position_bias_table_in_windows)

            if self.conv_route:
                router_k = nn.Conv2d(dim, self.num_heads, 3, stride=1, padding=1)
            else:
    ...
    def forward(self, x_all, mask_all=None):
        ...
        topKs = []
        x_k_route_scores_all = None
        for l_k in range(self.focal_level):        
            x_k = x_all[l_k]

            # x_k_global = x_k.mean(1, keepdim=True).mean(2, keepdim=True).repeat(1, x_k.shape[1], x_k.shape[2], 1)
            # x_k_local_global = torch.cat((x_k_global, x_k), 3)

            if self.conv_route:
                x_k4conv = x_k.permute(0, 3, 1, 2).contiguous()
                x_k_route_scores_k = self.routers[l_k](x_k4conv).view(B, self.num_heads, -1).transpose(1, 2)
            else:
                x_k_route_scores_k = self.routers[l_k](x_k)
                x_k_route_scores_k = x_k_route_scores_k.view(B, -1, self.num_heads)  # B, nH, nW, nHeads

            # x_k_route_scores = torch.softmax(x_k_route_scores, dim=1)

            if l_k == 0:
                x_k_route_scores_all = x_k_route_scores_k.view(B, x_k.shape[1], x_k.shape[2], self.num_heads)

            # add relative position bias before selection                  
            relative_position_range_k = self.relative_position_range_table_to_windows[l_k]
            relative_position_bias_table_to_windows_k = relative_position_range_k.unsqueeze(-1).repeat(1, 1, self.num_heads)

            if l_k == 0:
                relative_position_bias_table_to_windows_k = relative_position_bias_table_to_windows_k.view(
                    relative_position_bias_table_to_windows_k.shape[0], self.nWh, 1, self.nWw, 1, self.num_heads)                
                relative_position_bias_table_to_windows_k = relative_position_bias_table_to_windows_k.repeat(1, 1, self.window_size[0], 1, self.window_size[1], 1)
                relative_position_bias_table_to_windows_k = relative_position_bias_table_to_windows_k.view(relative_position_bias_table_to_windows_k.shape[0], -1, self.num_heads)                
            
            # out = x_k_route_scores_k.unsqueeze(1).repeat(1, 64, 1, 1)[relative_position_bias_table_to_windows_k.unsqueeze(0)==0]

            x_k_route_scores_positioned = x_k_route_scores_k.unsqueeze(1) + relative_position_bias_table_to_windows_k.unsqueeze(0)

            topK = min(self.topK, x_k_route_scores_positioned.shape[2]); topKs.append(topK)

            topk_score_k, topk_index_k = torch.topk(x_k_route_scores_positioned, topK, dim=2) # B, topK, nHeads
            ...
            k_k_selected = torch.gather(k_k, 1, topk_index_k.view(B, -1, self.num_heads).unsqueeze(-1).repeat(1, 1, 1, C // self.num_heads))
            v_k_selected = torch.gather(v_k, 1, topk_index_k.view(B, -1, self.num_heads).unsqueeze(-1).repeat(1, 1, 1, C // self.num_heads))

            k_k_selected = k_k_selected.view((-1,) + topk_index_k.shape[2:] + (C // self.num_heads,)).transpose(1, 2)
            v_k_selected = v_k_selected.view((-1,) + topk_index_k.shape[2:] + (C // self.num_heads,)).transpose(1, 2)

            k_all = torch.cat((k_all, k_k_selected.view(-1, self.num_heads, topK, C // self.num_heads)), 2)
            v_all = torch.cat((v_all, v_k_selected.view(-1, self.num_heads, topK, C // self.num_heads)), 2)
        
        self.route_scores = x_k_route_scores_all
        ...
        if x_k_route_scores_all is not None:
            route_scores = window_partition(x_k_route_scores_all, self.window_size[0]).permute(0, 3, 1, 2).contiguous().flatten(2).unsqueeze(2)
            attn[:, :, :window_area, :window_area] = attn[:, :, :window_area, :window_area] + route_scores

        # add relative position bias for patches inside a window
        if self.focal_level > 0:
            offset = window_area
            for k in range(self.focal_level):
                attention_bias_k = attention_bias[k].transpose(2, 3).unsqueeze(3).view(
                        -1, self.num_heads, 1, topKs[k]
                    )
                ...
                # if k == 0:
                #     relative_position_index_k = relative_position_index_k.view(
                #         relative_position_index_k.shape[0], self.nWh, 1, self.nWw, 1)                
                #     relative_position_index_k = relative_position_index_k.repeat(1, 1, self.window_size[0], 1, self.window_size[1])
                #     relative_position_index_k = relative_position_index_k.view(relative_position_index_k.shape[0], -1)                
                
                attention_index_k = attention_index[k].transpose(0, 1)
                attention_index_k = attention_index_k.view(self.nWh, 1, self.nWw, 1, B, topKs[k], self.num_heads)     
                attention_index_k = attention_index_k.repeat(1, self.window_size[0], 1, self.window_size[1], 1, 1, 1).view(-1, B, topKs[k], self.num_heads)

                relative_position_index_k_selected = torch.gather(relative_position_index_k, 1, attention_index_k.flatten(1)).view(attention_index_k.shape)
                ...
                relative_position_bias_to_windows = relative_position_bias_to_windows.transpose(0, 1)
                relative_position_bias_to_windows = relative_position_bias_to_windows.view(B, self.nWh, self.window_size[0], self.nWw, self.window_size[1], topKs[k], self.num_heads)
                relative_position_bias_to_windows = relative_position_bias_to_windows.permute(0, 1, 3, 2, 4, 5, 6).contiguous().view(B, self.nWh*self.nWw, self.window_size[0]*self.window_size[1], topKs[k], self.num_heads)
                
                relative_position_bias_to_windows = relative_position_bias_to_windows.view(-1, self.window_size[0]*self.window_size[1], topKs[k], self.num_heads)
                relative_position_bias_to_windows = relative_position_bias_to_windows.permute(0, 3, 1, 2).contiguous()
        ...
        x = (attn @ v_all).transpose(1, 2)

        # aggregate routing scores
        # if x_k_route_scores_all is not None:
        #     route_scores = window_partition(x_k_route_scores_all, self.window_size[0])
        #     route_scores = route_scores.view(route_scores.shape[0], -1, route_scores.shape[-1], 1)
        #     route_scores = torch.sigmoid(route_scores)            
        #     x = x * route_scores
