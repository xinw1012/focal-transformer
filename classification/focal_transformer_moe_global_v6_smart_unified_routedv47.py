...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, focal_factor, 
        ...
        self.pre_conv = nn.Linear(dim, 2*dim, bias=qkv_bias)
        self.v = nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0, groups=1, bias=qkv_bias)
        self.gate = nn.Sequential(
            nn.Conv2d(dim, 1, kernel_size=1, stride=1, padding=0, groups=1, bias=qkv_bias), 
            nn.Sigmoid()
        )
        # self.mid_conv = nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0, groups=1, bias=qkv_bias)
        # self.post_conv = nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0, groups=1, bias=qkv_bias)

        # self.linear = nn.Sequential(
        #     nn.Linear(dim*(1+self.focal_level+1), dim // 2, bias=qkv_bias), 
        #     nn.GELU(),
        ...
        if self.pool_method != "none":
            for k in range(self.focal_level):
                # kernel_size = min(self.focal_factor*k+3, 7)
                ...
                if self.pool_method == "conv":
                    ...
                    self.pool_layers.append(
                        nn.Sequential(
                            nn.Conv2d(dim, dim, kernel_size=kernel_size, stride=self.focal_stride, dilation=self.dilation, 
                            groups=dim, padding=self.dilation * (kernel_size // 2), padding_mode="zeros", bias=False),
                            # nn.BatchNorm2d(dim),
                            nn.GELU(),
                            )
                        )                                    
                ...
                elif self.pool_method == "pool":
                    self.pool_layers.append(
                        nn.Sequential(
                            nn.AvgPool2d(kernel_size=kernel_size, stride=self.focal_stride, 
    ...
    def forward(self, x_all, mask_all=None):
        ...
        x4conv_q, x4conv_kv = torch.split(x, (C, C), 1)
        # x4conv_kv = x4conv_kv.reshape(B, C, nH, nW)

        x4conv_q_all = x4conv_q
        x4conv_kv_all = x4conv_kv # * self.gate(x4conv_kv)
        if self.pool_method != "none": 
            # if we add coarser granularity and the pool method is not none
            for l in range(self.focal_level):         
                if self.pool_method in ["conv", "pool"]:
                    x4conv_kv = self.pool_layers[l](x4conv_kv)
                    x4conv_kv_all = x4conv_kv_all + x4conv_kv * self.gate(x4conv_kv)
                elif self.pool_method == "conv_share":
                    x4conv = self.pool_layers[l](x4conv.view(-1, 1, nH, nW)).view(B, C, nH, nW)
                    x_l = x4conv.permute(0, 2, 3, 1).contiguous() # B, nWh, nWw, C      
                if self.focal_stride > 1:
                    nH_l, nW_l = x_l.shape[1:3]
                    focal_factor = self.focal_stride*(l+1)
                    x_l = x_l[:,:,None,:,None,:].expand(-1, -1, focal_factor, -1, focal_factor, -1).contiguous().view(
                        B, nH_l*focal_factor, nW_l*focal_factor, C)
                    if x_l.shape != x_outs[0].shape:
                        if x_l.shape[1] < x_outs[0].shape[1]:
                            x_l = F.pad(x_l, (0,0,0,nW-nW_l*focal_factor,0,nH-nH_l*focal_factor))
                        else:
                            x_l = x_l[:, :nH, :nW]
                    else:
                        x_l = x_l.view(x_outs[0].shape)
        
        x_global = x4conv_kv.mean(2, keepdim=True).mean(3, keepdim=True)
        x4conv_kv_all = x4conv_kv_all + x_global * self.gate(x_global)
        x4conv_kv_all = self.v(x4conv_kv_all)
        x_out = x4conv_q_all * (1 + x4conv_kv_all)
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, img_size, patch_size=4, in_chans=3, embed_dim=96, use_conv_embed=False, norm_layer=nn.LayerNorm, use_pre_norm=False, is_stem=False):
        ...
        if self.use_conv_embed:
            ...
            self.reduction = nn.Linear(4 * in_chans, embed_dim, bias=False)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            # x = self.pre_norm(x)
            x = x.transpose(1, 2).view(x.shape[0], -1, self.input_resolution[0], self.input_resolution[1])
            # x = x.contiguous()
            x = self.downsample(x)
        return x
...
if __name__ == '__main__':
    ...
    # ckpt_path = "/datadrive/azureblobs/model_storage2/projects/focalv3/amlt-results/7357804646.75560-e120b629-de16-4558-b8b7-3d27ab62ea33/ckpt_epoch_290.pth"
