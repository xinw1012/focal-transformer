...
def get_topk_closest_indice(input_resolution, window_size_q, window_size_k, topk=1, dilation=1, offset=0, dist_norm="l2"):
    ...
    if topk >= relative_position_dists.shape[1]:
        # no need to sort and gather
        return torch.arange(relative_position_dists.shape[1]).view(1, -1).repeat(relative_position_dists.shape[0], 1), \
                relative_coords.permute(1, 2, 0).contiguous().float(), False
    else:
        ...
        return indice_topk, relative_coord_topk.permute(1, 2, 0).contiguous().float(), True
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, 
        ...
        for k in range(self.focal_level):
            ...
            topk_closest_indice, topk_closest_coord, rearranged = get_topk_closest_indice(
                input_resolution, self.window_size, (1, 1) if k == 0 else (self.focal_window, self.focal_window), 
                self.topK, # // (2**k), 
                dist_norm=dist_norm, 
                dilation=dilation, 
                offset=offset)
            self.rearranged.append(rearranged)
    ...
    def forward(self, x_all, mask_all=None):
        ...
        for l_k in range(self.focal_level):       
            ...
            if self.rearranged[l_k]:             
                ...
                topk_rpbs.append(topk_rpb_k)
                k_k_selected = k_k.unsqueeze(1).repeat(1, self.nWh*self.nWw, 1, 1, 1).view(-1, *k_k.shape[1:]).transpose(1, 2)
                v_k_selected = v_k.unsqueeze(1).repeat(1, self.nWh*self.nWw, 1, 1, 1).view(-1, *v_k.shape[1:]).transpose(1, 2)
...
class FocalTransformerBlock(nn.Module):
    ...
    def forward(self, x, x_pooled):
        ...
        x_windows_all = [x, x_pooled]
        x_window_masks_all = [self.attn_mask, self.attn_mask]
...
class PatchPooling(nn.Module):
    r""" Patch Pooling Layer.

    """
    def __init__(self, pool_size, input_resolution):
        super().__init__()
        self.pool_size = pool_size
        self.input_resolution = input_resolution
        self.pooler = nn.Linear(self.pool_size**2, 1)

    def forward(self, x):
        B, L, C = x.shape
        x = x.view(B, *self.input_resolution, C)
        x_windows = window_partition_noreshape(x, self.pool_size) # B, nw, nw, window_size, window_size, C    

        x_windows = x_windows.view(B, *x_windows.shape[1:3], self.pool_size*self.pool_size, C).transpose(3, 4) # B, nWh, nWw, C, wsize**2
        x_windows_pooled = self.pooler(x_windows).flatten(-2) # B, nWh, nWw, C                      

        return x_windows_pooled.view(B, -1, C)
...
class FocalTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        self.patch_poolers = nn.ModuleList()
        for i_layer in range(self.num_layers):
            ...
            patch_pooler = PatchPooling(window_size,                                
                                input_resolution=(patches_resolution[0] // (2 ** i_layer),
                                                 patches_resolution[1] // (2 ** i_layer))
                            )
            self.patch_poolers.append(patch_pooler)
    ...
    def forward_features(self, x):
        ...
        for k, layer in enumerate(self.layers):
            x_pooled = self.patch_poolers[k](x)
