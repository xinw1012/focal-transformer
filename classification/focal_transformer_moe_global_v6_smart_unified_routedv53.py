...
class FocalConvolution(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, focal_factor, 
        ...
        if self.pool_method != "none":
            for k in range(self.focal_level):
                ...
                kernel_size = self.focal_window if k == 0 else 9 # max(self.focal_window-self.focal_factor*k, 3)     
                    ...
                    if kernel_size < min(input_resolution):       
                        self.pool_layers.append(
                            nn.Sequential(
                                ...
                                groups=dim, padding=kernel_size//2 - stride // 2, padding_mode="zeros", bias=False),
                                # nn.BatchNorm2d(dim),
                                nn.GELU(),
                                )
                            )              
    ...
    def forward(self, x, mask_all=None):
        ...
        modulator = x_global*self.focal_weights[-1]
        ...
        if self.pool_method != "none": 
            ...
            for l in range(self.focal_level):         
                if self.pool_method in ["conv", "pool"]:
                    ...
                    if l == 0:                    
                        modulator = modulator + x4conv_kv*self.focal_weights[l]
                    else:
                        modulator_window = window_partition(modulator, self.focal_stride)
                        modulator_window = modulator_window + x4conv_kv.unsqueeze(-1).unsqueeze(-1)*self.focal_weights[l]
                        modulator = window_reverse(modulator_window, self.focal_stride, nH, nW)
