...
class FocalConvolution(nn.Module):
    ...
    def __init__(self, dim, input_resolution, expand_size, shift_size, window_size, focal_window, focal_factor, 
        ...
        self.gate = nn.Sequential(
            nn.Conv2d(dim, dim+1, kernel_size=1, stride=1, padding=0, groups=1, bias=qkv_bias), 
            # nn.Sigmoid(),
        )
        ...
        if self.pool_method != "none": 
            ...
            for l in range(self.focal_level):         
                if self.pool_method in ["conv", "pool"]:
                    ...
                    ctx = ctx + x4conv_kv*focal_weights[:, l:l+1]
        x_global = self.act(ctx.mean(2, keepdim=True).mean(3, keepdim=True))
        ctx = ctx + x_global*focal_weights[:,self.focal_level:]
        modulator = self.gate(ctx)
        modulator_multi, modulator_bias = modulator[:, :C], modulator[:, C:]
        x_out = x4conv_q*modulator_multi + modulator_bias
...
if __name__ == '__main__':
    ...
    model = FocalTransformer(
        img_size=img_size, 
        embed_dim=144, 
        depths=[4,4,28,4], drop_path_rate=0.5, 
